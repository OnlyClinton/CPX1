...
export async function recordAnalyticsEvent(input:AnalyticsEvent){
  ...
  const record={
    id,at,
    event:clean(input.event,100)||"unknown",
    sessionId:input.sessionId?clean(input.sessionId,160):null,
    anonymousUserId:input.anonymousUserId?clean(input.anonymousUserId,160):null,
    leadId:input.leadId?clean(input.leadId,160):null,
    vehicleId:input.vehicleId?clean(input.vehicleId,160):null,
    source:input.source?clean(input.source,120):null,
    medium:input.medium?clean(input.medium,120):null,
    campaign:input.campaign?clean(input.campaign,160):null,
    content:input.content?clean(input.content,160):null,
    referralCode:input.referralCode?clean(input.referralCode,160):null,
    pagePath:input.pagePath?clean(input.pagePath,300):null,
    landingPath:input.landingPath?clean(input.landingPath,300):null,
    referrer:input.referrer?clean(input.referrer,700):null,
    channel:input.channel?clean(input.channel,80):null,
    cta:input.cta?clean(input.cta,100):null,
