...
export async function recordVehicleAudit(input: VehicleAuditEvent) {
	...
	const record = {
		...
		mileage: Number.isFinite(Number(input.mileage))
			? Number(input.mileage)
			: null,
		stock: input.stock ? clean(input.stock, 80) : null,
		status: input.status ? clean(input.status, 40) : null,
		photoCount: Number.isFinite(Number(input.photoCount))
			? Number(input.photoCount)
			: null,
	...
	try {
		...
		console.error(
			"WDCC_VEHICLE_AUDIT_WRITE_FAILED",
			JSON.stringify({
