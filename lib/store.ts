...
async function readCanonicalSnapshot() {
	for (let attempt = 0; attempt < 3; attempt++) {
		...
		if (before.etag === after.etag)
			return {
				state: normalizeState(JSON.parse(raw.toString("utf8"))),
				raw,
				etag: after.etag,
			};
	}
...
export async function writeState(input: State) {
	...
	if (suppliedRevision !== current.state.revision)
		throw Error(
			`STATE_REVISION_CONFLICT:${suppliedRevision}:${current.state.revision}`,
		);
...
export function isQaVehicleRecord(vehicle: any) {
	const stock = String(vehicle?.stock || vehicle?.stock_id || "")
		.trim()
		.toUpperCase();
	...
	const badges = Array.isArray(vehicle?.badges)
		? vehicle.badges.map((value: any) => String(value || "").toUpperCase())
		: [];
	return (
		vehicle?.qa === true ||
