...
export function sessionCookieValue(user:User){
  return token(user);
}
export function sessionCookieHeader(user:User){
  return `${SESSION_COOKIE}=${sessionCookieValue(user)}; Path=/; Max-Age=${SESSION_MAX_AGE}; HttpOnly; Secure; SameSite=Strict`;
}
...
export async function setSession(user:User){
  const jar=await cookies();
  jar.set(SESSION_COOKIE,sessionCookieValue(user),{
    httpOnly:true,
    secure:true,
    sameSite:"strict",
    path:"/",
