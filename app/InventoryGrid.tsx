...
export default function InventoryGrid(){
  ...
  useEffect(()=>{
    ...
    fetch("/api/inventory",{cache:"no-store"})
      ...
      .then(body=>{
        if(!live)return;
        if(body?.previewFallback===true||body?.inventorySource==="last-known-good-real-proof"){setItems([]);setState("fallback");return}
        const vehicles=(Array.isArray(body?.items)?body.items:Array.isArray(body?.inventory)?body.inventory:[]).filter(customerVisible);
        setItems(vehicles);setState(vehicles.length?"ready":"empty");
      })
