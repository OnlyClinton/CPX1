    ...
    <section className={styles.grid} aria-live="polite">
      {shown.length?shown.map(v=>{
        ...
        return <article className={styles.card} key={v.id}>
          <Link prefetch={false} className={styles.photo} href={`/vehicle/${encodeURIComponent(v.id)}?source=r31-inventory-image`} aria-label={`View ${v.year} ${v.make} ${v.model}`}>
          ...
          <div className={styles.body}>
            ...
            <div className={styles.actions}><Link prefetch={false} href={`/vehicle/${encodeURIComponent(v.id)}?source=r31-inventory-details`}>VIEW DETAILS</Link><Link href={`/r31-preview/get-approved?source=r31-inventory-approved&vehicle=${encodeURIComponent(v.id)}`}>GET PRE-APPROVED</Link></div>
          </div>
        </article>;
