...
const text = (value: unknown, max: number) =>
	String(value ?? "")
		...
		.slice(0, max);
const LEAD_UPSTREAM = (
	process.env.WDCC_LEAD_UPSTREAM_URL ||
	"https://wdcc-lead-email-stage.vercel.app/api/lead"
).trim();
const DEALER_BACKEND = (
	process.env.WDCC_DEALER_BACKEND_URL || "https://wdcc-cpx-launch.vercel.app"
).replace(/\/$/, "");
...
function leadKind(body: any) {
	const raw = text(
		body?.kind ?? body?.type ?? body?.requestType,
		40,
	).toLowerCase();
	if (
		["schedule", "test-drive", "test_drive", "schedule-test-drive"].includes(
			raw,
		)
	)
		return "schedule";
	if (["contact", "call", "call-or-contact", "general"].includes(raw))
		return "contact";
	if (
		[
			"approval",
...
function notificationText(lead: any) {
	return [
		...
		`Lead ID: ${lead.id}`,
		lead.upstreamLeadId ? `Upstream ID: ${lead.upstreamLeadId}` : "",
	]
		.filter(Boolean)
		.join("\n");
}
function isQaLead(body: any, lead: any, idempotencyKey: string) {
	...
	return (
		body?.qa === true ||
		String(body?.qa || "").toLowerCase() === "true" ||
		idempotencyKey.toLowerCase().startsWith("wdcc-qa-") ||
		email.endsWith("@invalid.example") ||
		name.startsWith("WDCC QA ") ||
		name.startsWith("WDCC MATRIX") ||
		name.startsWith("WDCC ISOLATED") ||
		message.includes("automated wdcc contract verification")
	);
}

async function sendNotifications(lead: any) {
	const notifications: { email: string; sms: string; webhook: string } = {
	...
	const recipients = (
		process.env.WDCC_LEAD_NOTIFICATION_EMAILS || "bigcatscrap@gmail.com"
	)
		.split(",")
	...
	if (resendKey && recipients.length) {
		try {
			const response = await fetch("https://api.resend.com/emails", {
				...
				body: JSON.stringify({
					from:
						process.env.WDCC_LEAD_FROM_EMAIL ||
						"WDCC Leads <leads@wedontcarecars.com>",
					to: recipients,
	...
	const smsTo = process.env.WDCC_LEAD_NOTIFICATION_PHONE;
	if (twilioSid && twilioToken && twilioFrom && smsTo) {
		try {
			const form = new URLSearchParams({
				From: twilioFrom,
				To: smsTo,
				Body: notificationText(lead).slice(0, 1400),
			});
	...
	if (process.env.WDCC_LEAD_WEBHOOK_URL) {
		try {
			...
			notifications.webhook = response.ok
				? "sent"
...
async function persistViaDealer(
	...
	const response = await fetch(`${DEALER_BACKEND}/api/leads`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
	...
	if (!response.ok || !json?.ok || !json?.item?.id)
		throw Error(json?.error || `DEALER_LEAD_${response.status}`);
	return json;
}
async function persistViaUpstream(
	body: any,
	kind: string,
	idempotencyKey: string,
) {
	const payload = {
		...
		consent: true,
		source: body.utmSource || body.source || "wedontcarecars.com",
		idempotencyKey: idempotencyKey || undefined,
	...
	const response = await fetch(LEAD_UPSTREAM, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
			...(idempotencyKey ? { "Idempotency-Key": idempotencyKey } : {}),
	...
	const json = await response.json().catch(() => ({}));
	if (!response.ok || !json?.ok || !json?.leadId)
		throw Error(json?.error || `LEAD_UPSTREAM_${response.status}`);
	return {
		leadId: String(json.leadId),
		emailStatus: json.emailStatus || json.email || "upstream",
		smsStatus: json.smsStatus || json.sms || "upstream",
		mailto: json.mailto || null,
	};
}

function canonicalHost(req: Request) {
	return isDealerRuntime(req);
}
	...
	try {
		...
		const items = (
			String(user.role).toLowerCase() === "platform_admin"
				? state.leads
				: state.leads.filter(
						(lead) => String(lead.tenantId || "wdcc") === tenantId,
					)
		).sort((a, b) =>
...
export async function POST(req: Request) {
	try {
		...
		const normalized = {
			name: text(
				body?.name ??
					`${text(body?.firstName, 60)} ${text(body?.lastName, 60)}`,
				120,
			),
			phone: text(body?.phone, 40),
			email: text(body?.email, 160).toLowerCase(),
			vehicleInterest: text(
				body?.vehicleInterest ?? body?.vehicle ?? body?.vehicleId,
				240,
			),
			vehicleId: text(body?.vehicleId, 160),
			message: text(body?.message ?? body?.notes, 2000),
			preferredTime: text(body?.preferredTime ?? body?.appointmentTime, 120),
			source: text(body?.source, 80) || `cta-${kind || "unknown"}`,
			pagePath: text(body?.pagePath, 240),
		...
		if (!supportedKinds.has(kind))
			return NextResponse.json(
				{ ok: false, error: "valid_lead_type_required" },
				{ status: 400 },
			);
		if (!normalized.name)
			return NextResponse.json(
		...
		const idempotencyKey =
			text(req.headers.get("idempotency-key") ?? body?.idempotencyKey, 160) ||
			crypto.randomUUID();
		const qa = isQaLead(body, normalized, idempotencyKey);

		if (!canonicalHost(req)) {
			try {
				const dealer = await persistViaDealer(
					normalized,
					kind,
					idempotencyKey,
					qa,
				...
				return NextResponse.json(
					{ ...dealer, source: "dealer-ledger" },
					{
						status: dealer?.deduplicated ? 200 : 201,
					...
					return NextResponse.json(
						{
							...
							persisted: false,
							qa: true,
							error: "qa_dealer_persistence_failed",
							dealerError:
								dealerError instanceof Error
									? dealerError.message
									: "dealer_persistence_failed",
						},
						{ status: 503, headers: { "Cache-Control": "no-store" } },
					);
				const upstream = await persistViaUpstream(
					normalized,
					kind,
					idempotencyKey,
				);
				return NextResponse.json(
					{
						...
						item: {
							...
							status: "new",
							idempotencyKey,
							transport: "lead-upstream",
						},
						sync: {
							dealer: "failed",
							upstream: "synced",
							upstreamLeadId: upstream.leadId,
						},
						notifications: {
							email: upstream.emailStatus,
							sms: upstream.smsStatus,
							webhook: "upstream",
						},
						source: "lead-upstream",
						dealerError:
							dealerError instanceof Error
								? dealerError.message
								: "dealer_persistence_failed",
		...
		const state = await readState();
		if (idempotencyKey) {
			const existing: any = state.leads.find(
				(lead: any) => lead.idempotencyKey === idempotencyKey,
			);
			if (existing) {
				const existingQa =
					existing.qa === true ||
					existing.status === "test" ||
					isQaLead(existing, existing, idempotencyKey);
				if (!existingQa && existing?.sync?.upstream !== "synced") {
					try {
						const up = await persistViaUpstream(
							existing,
							existing.kind || kind,
							idempotencyKey,
						);
						existing.upstreamLeadId = up.leadId;
						existing.sync = {
							...(existing.sync || {}),
							upstream: "synced",
							upstreamLeadId: up.leadId,
		...
		const lead: any = {
			...
			...normalized,
			consent: true,
			consentVersion: "wdcc-request-v1",
			qa,
			status: qa ? "test" : "new",
			idempotencyKey,
			...
			createdAt: now,
			updatedAt: now,
			sync: { dealer: "saved", upstream: qa ? "suppressed_qa" : "pending" },
		};
		state.leads.push(lead);
		state.audit.push({
			id: crypto.randomUUID(),
			at: now,
			action: `lead.create.${qa ? "qa." : ""}${kind}`,
			actor: createdBy,
		...
		if (qa) {
			lead.notifications = {
				email: "suppressed_qa",
				sms: "suppressed_qa",
				webhook: "suppressed_qa",
			};
			lead.updatedAt = new Date().toISOString();
			saved = await writeState(state);
			return NextResponse.json(
				{
					ok: true,
					persisted: true,
		...
		try {
			const up = await persistViaUpstream(normalized, kind, idempotencyKey);
			lead.upstreamLeadId = up.leadId;
			lead.sync = {
				dealer: "saved",
				upstream: "synced",
				upstreamLeadId: up.leadId,
			...
			lead.sync = {
				...
				upstream: "pending",
				lastError:
					error instanceof Error ? error.message : "upstream_sync_failed",
			};
		}
