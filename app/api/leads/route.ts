...
async function emitPersistedLead(lead:any){
  if(lead?.qa)return;
  try{await recordAnalyticsEvent({event:"lead.persisted",at:lead.createdAt,sessionId:lead.sessionId||null,anonymousUserId:lead.anonymousUserId||null,leadId:lead.id||null,vehicleId:lead.vehicleId||null,source:lead.source||null,medium:lead.utmMedium||null,campaign:lead.utmCampaign||null,content:lead.utmContent||null,term:lead.utmTerm||null,clickId:lead.clickId||null,referralCode:lead.referralCode||null,pagePath:lead.pagePath||null,landingPath:lead.landingPath||null,referrer:lead.referrer||null,channel:"lead",cta:lead.cta||lead.kind||null,metadata:{kind:lead.kind,idempotencyKey:lead.idempotencyKey,firstTouch:{source:lead.firstSource||null,medium:lead.firstMedium||null,campaign:lead.firstCampaign||null,content:lead.firstContent||null,term:lead.firstTerm||null,clickId:lead.firstClickId||null,referralCode:lead.firstReferralCode||null}}});}catch(error){console.error("WDCC_LEAD_ANALYTICS_WRITE_FAILED",error);}
}
...
export async function POST(req:Request){
  try{
    ...
    const normalized={name:text(body?.name??`${text(body?.firstName,60)} ${text(body?.lastName,60)}`,120),phone:text(body?.phone,40),email:text(body?.email,160).toLowerCase(),vehicleInterest:text(body?.vehicleInterest??body?.vehicle??body?.vehicleId,240),vehicleId:text(body?.vehicleId,160),message:text(body?.message??body?.notes,2000),preferredTime:text(body?.preferredTime??body?.appointmentTime,120),source:text(body?.source,80)||`cta-${kind||"unknown"}`,cta:text(body?.cta,100),sessionId:text(body?.sessionId,160),anonymousUserId:text(body?.anonymousUserId,160),referralCode:text(body?.referralCode,160),landingPath:text(body?.landingPath,300),pagePath:text(body?.pagePath,240),referrer:text(body?.referrer,500),utmSource:text(body?.utmSource,120),utmMedium:text(body?.utmMedium,120),utmCampaign:text(body?.utmCampaign,160),utmContent:text(body?.utmContent,160),utmTerm:text(body?.utmTerm,160),clickId:text(body?.clickId,220),firstSource:text(body?.firstSource,120),firstMedium:text(body?.firstMedium,120),firstCampaign:text(body?.firstCampaign,160),firstContent:text(body?.firstContent,160),firstTerm:text(body?.firstTerm,160),firstClickId:text(body?.firstClickId,220),firstReferralCode:text(body?.firstReferralCode,160)};
    ...
    if(!canonicalHost(req)){
      try{const dealer=await persistViaDealer(normalized,kind,idempotencyKey,qa);const item=dealer?.item;if(item&&!qa)await emitPersistedLead(item);return NextResponse.json({...dealer,source:"dealer-ledger"},{status:dealer?.deduplicated?200:201,headers:{"Cache-Control":"no-store"}});}catch(dealerError){
        ...
        const item={id:upstream.leadId,kind,...normalized,consent:true,status:"new",idempotencyKey,transport:"lead-upstream",createdAt:new Date().toISOString(),qa:false};await emitPersistedLead(item);
        return NextResponse.json({ok:true,persisted:true,item,sync:{dealer:"failed",upstream:"synced",upstreamLeadId:upstream.leadId},notifications:{email:upstream.emailStatus,sms:upstream.smsStatus,webhook:"upstream"},source:"lead-upstream",dealerError:dealerError instanceof Error?dealerError.message:"dealer_persistence_failed"},{status:201,headers:{"Cache-Control":"no-store"}});
      }
    }

    const state=await readState();
    if(idempotencyKey){const existing:any=state.leads.find((lead:any)=>lead.idempotencyKey===idempotencyKey);if(existing){const existingQa=existing.qa===true||existing.status==="test"||isQaLead(existing,existing,idempotencyKey);if(!existingQa&&existing?.sync?.upstream!=="synced"){try{const up=await persistViaUpstream(existing,existing.kind||kind,idempotencyKey);existing.upstreamLeadId=up.leadId;existing.sync={...(existing.sync||{}),upstream:"synced",upstreamLeadId:up.leadId,syncedAt:new Date().toISOString()};existing.updatedAt=new Date().toISOString();await writeState(state);}catch{}}if(!existingQa)await emitPersistedLead(existing);return NextResponse.json({ok:true,persisted:true,deduplicated:true,qa:existingQa,item:existing,sync:existing.sync||{},notifications:existing.notifications||{},source:"local-ledger"},{status:200});}}
    ...
    if(!qa)await emitPersistedLead(lead);
