...
export async function POST(req:Request){
  ...
  try{
    ...
    if(!/^[A-Za-z0-9._-]{2,64}$/.test(username)){
      return NextResponse.json({ok:false,error:"valid_username_required"},{status:400});
    }
    if(password.length<8){
      return NextResponse.json({ok:false,error:"password_min_8"},{status:400});
    }
    if(!roles.has(role)){
    ...
    if(state.users.some((u:any)=>ids(u).some(x=>wanted.includes(x)))){
      return NextResponse.json({ok:false,error:"duplicate_identity"},{status:409});
    }

    const id=body?.certification===true ?
      nextCertification(state.users) : nextNormal(state.users);
    const now=new Date().toISOString();
    const user={
      id,email,secondaryEmail,username,
    ...
    state.audit.push({
      id:crypto.randomUUID(),at:now,action:"user.create",
      actor:String(actor.id??""),userId:id
    });
...
export async function PATCH(req:Request){
  ...
  try{
    ...
    if(action==="delete"){
      ...
      if(p.length<8)return NextResponse.json({ok:false,error:"password_min_8"},{status:400});
