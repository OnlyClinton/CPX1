...
function response(body: any, status: number, requestIdValue: string) {
	return NextResponse.json(body, {
		...
		headers: {
			...
			"X-WDCC-Request-ID": requestIdValue,
		},
	});
}
...
async function proxyPublicInventory(request: Request) {
	...
	const source = Array.isArray(json?.items)
		? json.items
	...
	return NextResponse.json(
		{ ...json, ok: true, count: items.length, items },
		{
			status: 200,
			headers: {
				"Cache-Control": "public, max-age=0, must-revalidate",
				"X-WDCC-Public-Inventory-Filter": "strict",
			},
		},
	);
}
...
export async function GET(request: Request) {
	if (!isDealerRuntime(request)) {
		const hasSession = String(request.headers.get("cookie") || "").includes(
			"__Host-wdcc_session=",
		);
		return hasSession
			? proxyDealer(request, "/api/inventory")
			: proxyPublicInventory(request);
	}
				...
				String(user.role).toLowerCase() === "platform_admin"
					...
					: state.vehicles.filter(
							(vehicle) =>
								...
								String(user.tenantId || "wdcc"),
						);
		} else items = publicVehicles(state).filter(publicEligible);
		return response(
			{ ok: true, count: items.length, items, revision: state.revision },
			200,
			rid,
		);
	} catch (error) {
...
export async function POST(request: Request) {
	...
	try {
		...
		if (
			...
			return fail("stock_number_already_exists", 409);
		...
		const item = {
			...
			status: "draft",
			photoPathnames: [],
			primaryPhotoPathname: null,
			createdAt: now,
			updatedAt: now,
			createdBy: user.id,
			uploadSource: "dealer-ui",
		};
		...
		return response(
			{ ok: true, item, revision: saved.revision, requestId: rid },
			201,
			rid,
		);
	} catch (error) {
