...
function normalizeStatus(value: unknown) {
	...
	if (["contacted", "contact_attempted", "attempted"].includes(status))
		return "contacted";
	if (["engaged", "working"].includes(status)) return "engaged";
	if (["qualified", "vehicle_matched"].includes(status)) return "qualified";
	if (["appointment", "appointment_set", "confirmed"].includes(status))
		return "appointment";
	if (["showed", "show"].includes(status)) return "showed";
	if (["deal", "deal_working", "finance", "funding"].includes(status))
		return "deal";
...
function priority(lead: any) {
	...
	return Math.min(
		99,
		recency + (kind === "schedule" ? 35 : kind === "approval" ? 30 : 20),
	);
}
...
export async function GET(request: Request) {
	if (!isDealerRuntime(request))
		return proxyDealer(request, "/api/crm/dashboard");
	const user = await currentUser();
	if (!user || !allowed.has(String(user.role || "").toLowerCase()))
	...
	try {
		...
		const leads = (
			platform
				? state.leads
				: state.leads.filter(
						(lead: any) => String(lead.tenantId || "wdcc") === tenantId,
					)
		)
			.map((lead: any) => ({
				...lead,
				pipelineStage: normalizeStatus(lead.status),
		...
		const allVehicles = platform
			? state.vehicles
			: state.vehicles.filter(
					(vehicle: any) => String(vehicle.tenantId || "wdcc") === tenantId,
				);
		...
		const pipeline = Object.fromEntries(
			stages.map((stage) => [
				stage,
				leads.filter((lead: any) => lead.pipelineStage === stage).length,
			]),
		);
		const now = Date.now();
		...
		const appointments = leads.filter(
			(lead: any) =>
				lead.pipelineStage === "appointment" ||
				String(lead.kind || "").toLowerCase() === "schedule",
		);
		const sold = leads.filter(
			(lead: any) => lead.pipelineStage === "sold",
		).length;
		return NextResponse.json(
			{
				ok: true,
				summary: {
					...
					hotLeads: leads.filter(
						(lead: any) =>
							lead.priority >= 70 &&
							!["sold", "lost"].includes(lead.pipelineStage),
					).length,
					appointments: appointments.length,
					sold,
					publishedInventory: published.length,
					totalInventory: allVehicles.length,
				},
				pipeline,
				leads: leads.slice(0, 100),
				hotLeads: leads
					.filter((lead: any) => !["sold", "lost"].includes(lead.pipelineStage))
					.slice(0, 8),
				inventory: published,
		...
		return NextResponse.json(
			{
				ok: false,
				error: error instanceof Error ? error.message : "crm_read_failed",
			},
