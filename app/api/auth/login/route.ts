...
const LOGIN_MAP:Record<string,string>={
  ...
  dealer:"dealer@internal.wedontcarecars.com",
};
...
function roleFor(email:string){return email.startsWith("admin@")?"platform_admin":"dealer";}
...
export async function POST(request:Request){
  try{
    ...
    const role=roleFor(login.email);
    ...
    return new Response(JSON.stringify({ok:true,role,user:{id:data?.user?.id||null,email:login.email,username:login.username,displayName:data?.user?.name||(role==="platform_admin"?"WDCC Admin":"WDCC Dealer"),role}}),{status:200,headers});
  }catch(error){
