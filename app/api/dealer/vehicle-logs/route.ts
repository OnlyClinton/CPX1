...
export async function GET(request: Request) {
	...
	const vehicleIds = new Set(
		(state?.vehicles || [])
			...
			.map((vehicle: any) => String(vehicle.id)),
	);
	const ledger = (state?.audit || [])
		.filter(
			(event: any) =>
				String(event.action || "").startsWith("vehicle.") &&
				(String(user.role).toLowerCase() === "platform_admin" ||
					!event.vehicleId ||
					vehicleIds.has(String(event.vehicleId))),
		)
		.slice(-250)
		.reverse();
	const visibleDurable = durable.filter(
		(event: any) =>
			String(user.role).toLowerCase() === "platform_admin" ||
			!event.vehicleId ||
			vehicleIds.has(String(event.vehicleId)),
