...
import {previewRecoveryMode} from "../../../lib/wdccPreviewRecoveryState";
...
export async function GET(){
  ...
  if(canonicalRuntime()){
    const recoveryMode=previewRecoveryMode();
    const storage=recoveryMode?{mode:"recovery-readonly"}:blobAuthority();
    ...
    if(storage.mode==="missing"||!session){
      return NextResponse.json({ok:false,degraded:true,service:"wdcc-canonical-backend",release:"WDCC-V53-OPS-HARDENED",backend:"local",storage:storage.mode,recoveryMode,session:session?"configured":"missing",state:"unverified",integrations:notificationIntegrations,provider:provider(),commit},{status:503,headers});
    }
    try{
      const state=await readState();
      return NextResponse.json({ok:true,degraded:false,service:"wdcc-canonical-backend",release:"WDCC-V53-OPS-HARDENED",backend:"local",storage:storage.mode,recoveryMode,session:"configured",state:"readable",revision:state.revision,integrations:notificationIntegrations,provider:provider(),commit},{status:200,headers});
    }catch(error){
      return NextResponse.json({ok:false,degraded:true,service:"wdcc-canonical-backend",release:"WDCC-V53-OPS-HARDENED",backend:"local",storage:storage.mode,recoveryMode,session:"configured",state:"unreadable",integrations:notificationIntegrations,error:error instanceof Error?error.message:"state_read_failed",provider:provider(),commit},{status:503,headers});
    }
  }
