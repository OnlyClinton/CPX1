...
export default function PortalExperience({mode}:{mode:Mode}){
  ...
  return <><style jsx global>{portalCss}</style><main className="wdccPortalApp">
    <header className="wdccPortalTopbar"><div className="wdccPortalBrand"><img src="/wdcc-official-logo.webp" alt="WDCC"/><div><strong>{mode==="admin"?"WDCC ADMIN PORTAL":"WDCC DEALER PORTAL"}</strong><span>{session?.user?.displayName||session?.user?.username||"Authorized user"}</span></div></div><button onClick={logout}>SIGN OUT</button></header>
