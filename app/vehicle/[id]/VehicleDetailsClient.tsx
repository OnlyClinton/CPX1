...
export default function VehicleDetailsClient({id,allowVisualFixture=false}:{id:string;allowVisualFixture?:boolean}){
  const[vehicle,setVehicle]=useState<VehicleRecord|null>(null);
  const[state,setState]=useState<LoadState>("loading");
  const[fixtureMode,setFixtureMode]=useState(false);

  useEffect(()=>{
    if(!id){setVehicle(null);setFixtureMode(false);setState("error");return;}
    ...
    let live=true;
    setFixtureMode(false);setState("loading");
    fetch(`/api/inventory/${encodeURIComponent(id)}`,{cache:"no-store",signal:controller.signal})
      .then(async response=>{const body=await response.json().catch(()=>({}));const nonCanonical=body?.live===false||body?.previewFallback===true||body?.recoveryFallback===true||body?.inventorySource==="last-known-good-real-proof"||body?.inventorySource==="verified-recovery-readonly";if(!response.ok||nonCanonical)throw new Error(body?.error||`Vehicle ${response.status}`);const item=body?.item||body?.vehicle;if(!item||!customerVisible(item))throw new Error("VEHICLE_NOT_PUBLIC");return item})
      .then(item=>{if(live){setVehicle(item);setState("ready")}})
      .catch(error=>{if(live&&error?.name!=="AbortError"){setVehicle(null);setState("error")}});
    return()=>{live=false;controller.abort()};
  },[id,allowVisualFixture]);

  const v=vehicle;
  const title=v?`${v.year} ${v.make} ${v.model}`:"VEHICLE DETAILS";
  const listingLabel=v?`${title}${v.trim?` ${v.trim}`:""}`:title;
  const q=id?`?vehicle=${encodeURIComponent(id)}&vehicleLabel=${encodeURIComponent(listingLabel)}&source=vdp`:"";
    ...
    <main className="vehiclePage wdcc-public-page">
      ...
      <section className="section light vehicleSection"><div className="wrap">
        ...
        {state==="ready"&&v&&<div className="vehicleLayout">
          <section className="vehicleMedia"><div className="photo vehiclePhoto">{src?<img src={src} alt={title}/>:<div className="vehiclePhotoMissing" role="img" aria-label={`${title} photo temporarily unavailable`}><span>PHOTO TEMPORARILY UNAVAILABLE</span></div>}</div><p>{fixtureMode?"Historical record photo is not being substituted while media storage is blocked.":"Photos shown are attached to this published vehicle record."}</p></section>
