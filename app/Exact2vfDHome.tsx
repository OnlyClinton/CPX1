...
export default function Exact2vfDHome(){
 ...
 return <div className="wdcc-app screenshotContract">
  ...
  <section className="inventory-showcase screenshotInventory"><div className="section-heading"><div><span className="section-kicker">Featured inventory</span><h2>Vehicles ready now.</h2><p className="section-deck">Cash price and down payment shown clearly.</p></div><Link className="text-link" href="/inventory">View all inventory →</Link></div>
    <div className="featured-carousel"><button className="carousel-arrow carousel-prev" onClick={()=>move(-1)} aria-label="Previous vehicle">‹</button><div className="featured-grid">{visible.map((v,i)=>{const down=Number(v.downPayment??v.down_payment??0),tags=[v.bodyStyle||v.body_style,v.transmission,v.drivetrain].filter(Boolean).slice(0,3),label=badge(v);return <div className={`featured-slide${i===active?" active":""}`} key={String(v.id||v.slug||i)}><article className="vehicle-card"><Link className="vehicle-image" href={href(v)}><img src={photo(v)} alt={`${v.year} ${v.make} ${v.model}`}/><span className="card-badges"><span className={badgeTone(label)}>{label}</span></span></Link><div className="vehicle-card-body"><p className="eyebrow">{v.year} {v.make}</p><Link className="vehicle-title" href={href(v)}>{v.model}{v.trim?` ${v.trim}`:""}</Link><strong className="vehicle-price">${Number(v.price||0).toLocaleString()}</strong><p className="vehicle-payment">{down?`$${down.toLocaleString()} down`:"Call for down payment"} • {Number(v.mileage||0).toLocaleString()} miles</p><div className="spec-pills">{tags.map((t,j)=><span key={j}>{String(t)}</span>)}</div><Link className="vehicle-card-cta" href={href(v)}>View details →</Link></div></article></div>})}</div><button className="carousel-arrow carousel-next" onClick={()=>move(1)} aria-label="Next vehicle">›</button></div>
  </section>

  <section className="how-section screenshotHow" id="how-it-works"><div className="section-heading"><div><span className="section-kicker">One simple process. No hoops. No hassle.</span><h2>In-house financing <strong>made easy.</strong></h2></div><Link className="startApproval" href="/get-approved?source=how-it-works">Start pre-approval →</Link></div><div className="steps-grid"><article><span className="step-number">1</span><span className="step-icon">▤</span><div><h3>Apply online</h3><p>Send basic details securely.</p></div></article><article><span className="step-number">2</span><span className="step-icon">◫</span><div><h3>Talk to Sean</h3><p>Confirm down payment and vehicle fit.</p></div></article><article><span className="step-number">3</span><span className="step-icon">▣</span><div><h3>Choose your car</h3><p>Shop our inventory online or in person.</p></div></article><article><span className="step-number">4</span><span className="step-icon">⌕</span><div><h3>Drive today</h3><p>Schedule pickup or test drive.</p></div></article></div></section>
