...
const fallback:Vehicle[]=[
  {id:"recovered-2004-nissan-350z",slug:"2004-nissan-350z",year:2004,make:"Nissan",model:"350Z",price:4900,downPayment:2000,mileage:154000,image:"/assets/cars/2004-nissan-350z-1.webp",bodyStyle:"Car",drivetrain:"RWD"},
  {id:"recovered-2016-ford-f150-limited",slug:"2016-ford-f150-limited",year:2016,make:"Ford",model:"F-150",trim:"Limited",price:15000,downPayment:6000,mileage:164000,image:"/assets/cars/2016-ford-f150-limited-1.webp",bodyStyle:"Truck",transmission:"Automatic"},
  {id:"recovered-2019-honda-pilot",slug:"2019-honda-pilot",year:2019,make:"Honda",model:"Pilot",price:7900,downPayment:3000,mileage:380000,image:"/assets/cars/2019-honda-pilot-1.webp",bodyStyle:"SUV",transmission:"Automatic"},
  {id:"recovered-2019-kia-sportage",slug:"2019-kia-sportage",year:2019,make:"Kia",model:"Sportage",price:6500,downPayment:2500,mileage:127000,image:"/assets/cars/2019-kia-sportage-1.webp",bodyStyle:"SUV",transmission:"Automatic"},
  {id:"recovered-2019-toyota-rav4",slug:"2019-toyota-rav4",year:2019,make:"Toyota",model:"RAV4",price:10500,downPayment:4500,mileage:240000,image:"/assets/cars/2019-toyota-rav4-1.webp",bodyStyle:"SUV",transmission:"Automatic"}
];
...
function href(v:Vehicle){const id=String(v.id||"");return id?`/vehicle/${encodeURIComponent(id)}`:"/inventory"}
  ...
  return <div className={appClass}>
    ...
    <div className="header-shell home-header-shell">
      ...
      <header className="site-header">
        <button type="button" className="mobile-menu" aria-expanded={open} aria-label="Open navigation" onClick={()=>setOpen(v=>!v)}><span aria-hidden="true"><i/><i/><i/></span></button>
        ...
        <nav className={`main-nav${open?" open":""}`} aria-label="Main navigation">
          <Link href="/inventory">Inventory</Link><Link href="/get-approved?source=nav-financing">Financing</Link><Link href="/#how-it-works">How it works</Link><Link href="/#reviews">Reviews</Link><Link href="/#about">About us</Link>
        </nav>
        <Link id="apply" className="btn btn-primary header-cta" href="/get-approved?source=header-get-approved">Get pre-approved</Link>
        <TrackedCallLink className="mobile-call" source="mobile-header-call" label="Call Sean at 813-516-4752"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7.2 3.5 10 7.8 8.4 9.4c1.1 2.2 2.9 4 5.1 5.1l1.6-1.6 4.4 2.8-.7 3.7c-.2.8-.9 1.4-1.8 1.4C9.4 20.2 3.8 14.6 3.2 7c-.1-.9.5-1.6 1.4-1.8l2.6-.7Z"/></svg><span><small>Call</small>Sean</span></TrackedCallLink>
      </header>
    </div>
    ...
    <section className="hero" style={{"--hero-image":"url(/wdcc-hero-v2.webp)"} as CSSProperties}>
      <div className="hero-shade"/><div className="hero-copy"><p className="hero-kicker">Tampa Bay · Drive today</p>
        ...
        <div className="hero-actions" aria-label="Choose your next step">
          ...
          <TrackedCallLink className="hero-call hero-cta-contact" source="hero-call-sean" label="Call Sean"><span><small>Questions or ready?</small><strong>Call Sean</strong></span><b aria-hidden="true">→</b></TrackedCallLink>
        </div>
    ...
    <section className="inventory-showcase">
      ...
      <div className="featured-carousel" aria-roledescription="carousel" aria-label="Featured vehicles">
        <button type="button" className="carousel-arrow carousel-prev" aria-label="Previous featured vehicle" onClick={()=>move(-1)}>‹</button>
        ...
        <button type="button" className="carousel-arrow carousel-next" aria-label="Next featured vehicle" onClick={()=>move(1)}>›</button>
        <div className="carousel-dots" role="tablist" aria-label="Choose featured vehicle">{visible.map((v,i)=><button type="button" key={String(v.id||i)} className={i===active?"active":""} aria-label={`Show ${v.year} ${v.make} ${v.model}`} aria-selected={i===active} role="tab" onClick={()=>setActive(i)}/>)}</div>
      </div>
    </section>
    ...
    <footer className="site-footer" id="about"><span className="brand-logo compact"><img src="/wdcc-logo-transparent.webp" alt="We Don't Care Cars" width="512" height="512"/></span><div><strong>WDCC · We Don't Care Cars</strong><span>Serving Tampa Bay · Confirm availability before visiting</span></div><TrackedCallLink source="footer-phone" label="Call Sean at 813-516-4752">813-516-4752</TrackedCallLink><span className="footer-links"><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></span></footer>
    <div className="mobile-action-bar" aria-label="Quick actions"><Link className="mobile-dock-drive" href="/schedule-test-drive?source=schedule-test-drive">Test drive</Link><Link className="mobile-dock-qualify" href="/get-approved?source=get-approved">Get qualified</Link><TrackedCallLink className="mobile-dock-contact" source="mobile-dock-call" label="Call Sean">Call Sean</TrackedCallLink></div>
  </div>;
}
