...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const done=window.setTimeout(()=>{document.documentElement.classList.remove("wdcc-intro-active");setPhase("done")},1320);
  ...
  return <div className={`li li-${phase}`} aria-label="WDCC opening intro">
    <style>{`
      ...
      .li{position:fixed;inset:0;z-index:2147483000;overflow:hidden;background:#02070c;isolation:isolate;opacity:1;transition:opacity .26s ease}
      ...
      .li-scene img{display:block;width:100%;height:100%;object-fit:cover;object-position:67% 46%;filter:saturate(1.18) contrast(1.06) brightness(.94);transform:none!important;animation:none!important;transition:none!important}
      ...
      .li-badge{position:absolute;z-index:4;left:50%;top:46%;width:min(25vw,238px);aspect-ratio:1;transform:translate(-50%,-50%)!important;display:grid;place-items:center;border-radius:50%;background:#fff;box-shadow:0 18px 52px rgba(0,0,0,.68),0 0 0 3px rgba(255,255,255,.72);animation:none!important;transition:none!important}
      .li-badge img{display:block;width:94%;height:94%;object-fit:contain;border-radius:50%;clip-path:circle(48% at 50% 50%);background:transparent;transform:none!important;animation:none!important;transition:none!important}
      ...
      @media(max-width:1365px){
        .li-scene img{position:absolute;width:min(118vw,1100px);height:auto;max-width:none;left:50%;top:7vh;transform:translateX(-50%)!important;object-fit:contain;object-position:center;filter:saturate(1.2) contrast(1.06) brightness(.98)}
        .li-scene:after{background:linear-gradient(180deg,rgba(2,7,12,0) 0%,rgba(2,7,12,.01) 38%,rgba(2,7,12,.18) 53%,rgba(2,7,12,.68) 68%,#02070c 82%,#02070c 100%)}
      ...
      @media(max-width:520px){
        .li-scene img{width:122vw;top:9vh}
