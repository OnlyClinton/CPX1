...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const done=window.setTimeout(()=>{document.documentElement.classList.remove("wdcc-intro-active");setPhase("done")},1280);
  ...
  return <div className={`li li-${phase}`} aria-label="WDCC opening intro">
    <style>{`
      ...
      .li-scene img{display:block;width:100%;height:100%;object-fit:cover;object-position:68% 42%;filter:saturate(1.18) contrast(1.02) brightness(1.17);transform:none!important;animation:none!important;transition:none!important}
      ...
      .li-tag{position:absolute;z-index:5;left:50%;top:63%;transform:translateX(-50%);margin:0;color:#fff;white-space:nowrap;font:900 clamp(12px,1.25vw,15px)/1 system-ui,sans-serif;letter-spacing:.11em;text-transform:uppercase;text-shadow:0 2px 12px rgba(0,0,0,.72);opacity:1}
