...
async function headerTarget():Promise<Rect>{
  for(let i=0;i<90;i++){
...
async function imageBitmap(url:string){const r=await fetch(url,{cache:"force-cache"});return createImageBitmap(await r.blob())}
...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const complete=()=>{if(cancelled)return;document.documentElement.classList.remove("wdcc-intro-active");setDone(true)};

    (async()=>{
      const target=await headerTarget(); if(cancelled)return;
      const vw=innerWidth,vh=innerHeight;
      const size=Math.min(vw<600?vw*.56:260,260);
      const start:Rect={x:(vw-size)/2,y:vh*.40-size/2,width:size,height:size};
      ...
      try{
        ...
        const ctx=(canvas as any).getContext("webgpu"); const format=gpu.getPreferredCanvasFormat();
        const dpr=Math.min(devicePixelRatio||1,2); canvas.width=Math.round(vw*dpr);canvas.height=Math.round(vh*dpr);
        ctx.configure({device,format,alphaMode:"premultiplied"});
        const bmp=await imageBitmap("/wdcc-official-logo.webp");
        ...
        const shader=device.createShaderModule({code:`
          ...
          @vertex fn vs(@builtin(vertex_index)i:u32)->O{
            ...
            let a=q[i];let px=u.rect.xy+a*u.rect.zw;let ndc=vec2f(px.x/u.viewport.x*2.0-1.0,1.0-px.y/u.viewport.y*2.0);
            var o:O;o.pos=vec4f(ndc,0,1);o.uv=a;return o;
          }
        ...
        const uniform=device.createBuffer({size:32,usage:72});
        ...
        await wait(60);complete();
      }catch{
        if(!fallback){complete();return}
        ...
        scene?.animate([{opacity:1},{opacity:0}],{duration,easing:"ease-out",fill:"forwards"});
        await anim.finished.catch(()=>{});await wait(60);complete();
      }
    })();
  ...
  return <div className="li gpu-intro" aria-label="WDCC opening intro">
    <style>{`
      ...
      .li-badge{position:absolute;inset:0;z-index:4;pointer-events:none;background:transparent!important;box-shadow:none!important;animation:none!important}.li-badge img{position:absolute;display:block;object-fit:contain;border-radius:50%;clip-path:circle(48% at 50% 50%);opacity:1;animation:none!important;transition:opacity .08s linear;will-change:left,top,width,height}
      ...
      @media(max-width:600px){.li-scene img{object-position:69% 42%;filter:saturate(1.2) contrast(1.01) brightness(1.24)}}
    `}</style>
