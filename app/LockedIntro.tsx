...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const frame=(now:number)=>{
      ...
      if(scene&&renderer&&camera){const settle=1-ease(Math.max(0,(elapsed-.28)/1.02));for(let i=0;i<fxObjects.length;i++){const o=fxObjects[i];if(o.userData?.base){o.position.x+=o.userData.vx*(dt*1000);o.position.y+=o.userData.vy*(dt*1000);o.material.opacity=o.userData.base*settle}else{o.material.opacity*=.992}}try{renderer.render(scene,camera)}catch{}}
      if(elapsed<1.76)raf=requestAnimationFrame(frame)
    };
