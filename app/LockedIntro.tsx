...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const done=window.setTimeout(()=>{document.documentElement.classList.remove("wdcc-intro-active");setPhase("done")},2260);
  ...
  return <div className={`li li-${phase}`} aria-label="WDCC opening intro">
    <style>{`
      ...
      .li{position:fixed;inset:0;z-index:2147483000;overflow:hidden;background:#050d14;isolation:isolate;opacity:1;transition:opacity .26s cubic-bezier(.22,.72,.18,1) .10s}
      .li-exit{opacity:0;pointer-events:none}
      .li-scene{position:absolute;inset:0;z-index:1;overflow:hidden;pointer-events:none;background:#050d14;opacity:1;transition:opacity .12s linear}
      .li-exit .li-scene{opacity:0}
      ...
      .li-exit .li-smoke{opacity:0!important;animation:none!important;transition:opacity .08s linear!important}
      ...
      .li-exit .li-badge,.li-exit .li-tag,.li-exit .li-skip{opacity:0!important;animation:none!important;pointer-events:none!important}
    ...
    <div className="li-badge"><img data-wdcc-intro-badge-art="owner-approved" src={WDCC_CORRECT_LOGO_DATA_URI} alt="We Don't Care Cars" width="128" height="128"/></div>
