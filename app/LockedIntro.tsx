...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const exit=window.setTimeout(()=>setPhase("exit"),2380);
    const done=window.setTimeout(()=>{document.documentElement.classList.remove("wdcc-intro-active");setPhase("done")},2840);
  ...
  return <div className={`li li-${phase}`} aria-label="WDCC opening intro">
    <style>{`
      ...
      .li{position:fixed;inset:0;z-index:2147483000;overflow:hidden;background:#050d14;isolation:isolate;opacity:1;transition:opacity .46s cubic-bezier(.22,.72,.18,1)}
      ...
      .li-smoke{position:absolute;inset:-8%;z-index:3;overflow:hidden;pointer-events:none;opacity:1;background:
        radial-gradient(ellipse at 18% 40%,rgba(236,242,247,.26) 0%,rgba(181,196,207,.12) 25%,transparent 54%),
        radial-gradient(ellipse at 80% 55%,rgba(225,234,241,.20) 0%,rgba(148,168,182,.10) 31%,transparent 59%),
        linear-gradient(180deg,rgba(15,31,43,.09),rgba(5,13,20,.22));
        backdrop-filter:blur(15px) saturate(.86) contrast(.96);-webkit-backdrop-filter:blur(15px) saturate(.86) contrast(.96);
        will-change:opacity,backdrop-filter;animation:liSmokeClear 1.08s cubic-bezier(.16,.80,.24,1) .10s forwards}
      ...
      .li-smoke:before{background:
        radial-gradient(ellipse at 32% 52%,rgba(245,248,250,.24) 0%,rgba(184,199,210,.12) 31%,transparent 61%),
        radial-gradient(ellipse at 68% 36%,rgba(226,235,241,.20) 0%,rgba(170,188,201,.10) 29%,transparent 58%);
        filter:blur(19px);animation:liFogDrift 1.72s cubic-bezier(.18,.72,.20,1) .06s forwards}
      ...
      @media(max-width:600px),(max-width:1180px) and (hover:none) and (pointer:coarse){
        .li-scene img{object-position:61% 45%;filter:saturate(1.25) contrast(1.02) brightness(1.30)}
        html body .li .li-badge{top:38.5%!important;width:min(70vw,276px)!important;min-width:238px!important}
        html body .li .li-tag{top:59%!important;font-size:13px!important;letter-spacing:.06em!important;padding:9px 13px!important}
        .li-smoke{inset:-10%;backdrop-filter:blur(16px) saturate(.84);-webkit-backdrop-filter:blur(16px) saturate(.84)}
      }
