...
export default function LockedIntro(){
  ...
  useEffect(()=>{
    ...
    const exit=window.setTimeout(()=>setPhase("exit"),900);
    const done=window.setTimeout(()=>{document.documentElement.classList.remove("wdcc-intro-active");setPhase("done")},1240);
  ...
  return <div className={`li li-${phase}`} aria-label="WDCC opening intro">
    <style>{`
      ...
      .li-badge{position:absolute;z-index:4;left:50%;top:46%;width:min(24vw,218px);aspect-ratio:1;transform:translate(-50%,-50%);display:grid;place-items:center;border-radius:50%;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(0,0,0,.66),0 0 0 2px rgba(255,255,255,.65);opacity:1}
      .li-badge img{content:url('/wdcc-official-logo.webp')!important;display:block;width:94%;height:94%;object-fit:contain;border-radius:50%;clip-path:circle(48% at 50% 50%);background:transparent}
      ...
      @media(max-width:1100px),(hover:none) and (pointer:coarse){
        ...
        .li-badge{top:45%;width:min(46vw,178px)}
