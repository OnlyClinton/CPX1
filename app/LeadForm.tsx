...
export default function LeadForm({
	...
	async function submit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const body: Record<string, FormDataEntryValue> = Object.fromEntries(
				form.entries(),
			);
			...
			formEl.reset();
			setMessage(
				json?.sync?.upstream === "pending"
					? "Received. Your request is saved and queued for sync; Sean's team can see it now."
					: "Received. Sean's team will follow up shortly.",
			);
		} catch (error) {
			setMessage(
				error instanceof Error && error.message !== "submit_failed"
					? error.message
					: "We couldn't send this request. Please call 813-516-4752.",
			);
		} finally {
			setBusy(false);
		}
		...
		<form className="leadForm" onSubmit={submit}>
			<div className="leadGrid">
				...
				{kind !== "contact" && (
					<label>
						Vehicle of interest
						<input name="vehicleInterest" maxLength={240} />
					</label>
				)}
				{kind === "schedule" && (
					<label>
						Preferred date or time
						<input name="preferredTime" maxLength={120} />
					</label>
				)}
				...
				<label className="consent wide">
					<input name="consent" type="checkbox" required /> I agree WDCC may
					call, text, or email me about this request at the contact information
					I provided. Consent is not a condition of purchase. Message and data
					rates may apply.
				</label>
			</div>
			<button className="cta red" disabled={busy} type="submit">
				{busy ? "SENDING…" : "SEND REQUEST"}
			</button>
			{message && (
				<output className="leadMessage" aria-live="polite">
					{message}
				</output>
			)}
		</form>
