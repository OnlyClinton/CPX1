...
export default function DealerInventory(){
  ...
  useEffect(()=>{const requested=new URLSearchParams(window.location.search).get("view");if(requested&&filters.includes(requested))setFilter(requested);load().catch(e=>setMessage(e.message||"Inventory list failed"))},[]);
  const shown=useMemo(()=>items.filter(v=>filter==="all"?true:String(v.status||"").toLowerCase()===filter),[items,filter]);
  const selectFilter=(view:string)=>{setFilter(view);const url=new URL(window.location.href);url.searchParams.set("view",view);window.history.replaceState({},"",url.toString())};
  return <main className="crmShell"><aside className="crmSidebar"><Link className="crmLogo" href="/dealer"><img src="/wdcc-official-logo.webp" alt="WDCC"/><span>SALES COMMAND</span></Link><nav><Link href="/dealer">Today</Link><Link href="/dealer/leads">Leads</Link><Link className="active" href="/dealer/inventory">Inventory</Link><Link href="/dealer/inventory/new">+ Add Vehicle</Link><Link href="/dealer/inventory/logs">Vehicle Logs</Link><Link href="/">View Website</Link></nav></aside><section className="crmMain">
    ...
    <div className="leadInboxTabs">{[["all","All"],["published","Published"],["draft","Draft"],["archived","Archived"]].map(([k,l])=><button key={k} className={filter===k?"active":""} onClick={()=>selectFilter(k)}>{l} · {k==="all"?items.length:items.filter(v=>String(v.status||"").toLowerCase()===k).length}</button>)}</div>
