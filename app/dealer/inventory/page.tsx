...
const customerVisible=(v:any)=>String(v?.status||"").toLowerCase()==="published"&&!isQa(v)&&Number(v?.year)>1900&&Boolean(String(v?.make||"").trim())&&Boolean(String(v?.model||"").trim())&&Number(v?.price)>0;
...
export default function DealerInventory(){
  ...
  return <main className="refDealerShell">
    <aside className="refDealerSide">
      ...
      <nav><Link href="/dealer">Dashboard</Link><div className="refDealerLabel">INVENTORY</div><Link className="active" href="/dealer/inventory">All Vehicles</Link><Link href="/dealer/inventory/new">＋ Add / Edit Vehicle</Link><Link href="/dealer/inventory?status=live">Customer Live</Link><Link href="/dealer/inventory?status=attention">Needs Attention</Link><div className="refDealerLabel">OPERATIONS</div><Link href="/dealer/leads">Leads</Link><Link href="/dealer/leads?view=appointments">Appointments</Link><Link href="/dealer/leads?view=appointments">Test Drives</Link><Link href="/dealer/leads">Customers</Link><Link href="/dealer/leads">Applications</Link><Link href="/dealer/leads">Messages</Link><Link href="/dealer/inventory/logs">Reports</Link><Link href="/dealer">Settings</Link></nav>
    ...
    <section className="refDealerBody">
      ...
      <div className="refDealerContent">
        <div className="refDealerTitle"><div><h1>{viewTitle(status)}</h1><p>{status==="live"?`${shown.length} genuinely customer-visible vehicles.`:status==="qa"?`${shown.length} internal QA vehicles hidden from shoppers.`:"Manage dealer inventory, publish readiness and visibility."}</p></div><div className="refDealerTitleActions"><Link className="primary" href="/dealer/inventory/new">＋ Add / Edit Vehicle</Link><button>⇧ Import</button><button>⇧ Export</button></div></div>
        <div className="refDealerStats"><article><span>Total Vehicles</span><b>{items.length}</b></article><article><span>Customer Live</span><b>{live}</b><em>{items.length?Math.round(live/items.length*100):0}%</em></article><article><span>Published</span><b>{published}</b></article><article><span>Drafts</span><b>{drafts}</b></article><article><span>Needs Attention</span><b>{attention}</b></article><article><span>QA Hidden</span><b>{qa}</b></article></div>
        <div className="refDealerFilters"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by make, model, year, or stock #…"/><select><option>All Makes</option></select><select><option>All Models</option></select><select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All Views</option><option value="live">Customer Live</option><option value="published">All Published</option><option value="draft">Draft</option><option value="attention">Needs Attention</option><option value="qa">QA / Customer Hidden</option><option value="archived">Archived</option></select><button>☷ Filters</button><button>⇅ Sort</button></div>
