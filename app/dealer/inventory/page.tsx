...
export default function DealerInventory(){
  ...
  useEffect(()=>{const requested=new URLSearchParams(location.search).get("status");if(requested&&filters.includes(requested))setFilter(requested);load().catch(e=>setMessage(e.message||"Inventory list failed"))},[]);
  function chooseFilter(next:string){setFilter(next);const url=new URL(location.href);url.searchParams.set("status",next);history.replaceState(null,"",url.pathname+url.search)}
  const shown=useMemo(()=>items.filter(v=>filter==="all"?true:String(v.status||"").toLowerCase()===filter),[items,filter]);
  return <main className="crmShell"><aside className="crmSidebar"><Link className="crmLogo" href="/dealer"><img src="/wdcc-official-logo.webp" alt="WDCC"/><span>SALES COMMAND</span></Link><nav><Link href="/dealer">Today</Link><Link href="/dealer/leads">Leads</Link><Link className="active" href="/dealer/inventory">Inventory</Link><Link href="/dealer/inventory/new">+ Add Vehicle</Link><Link href="/dealer/inventory/logs">Vehicle Logs</Link><Link href="/">View Website</Link></nav></aside><section className="crmMain">
    <div className="leadInboxHeader"><div><span className="crmKicker">VEHICLE INVENTORY</span><h1>Inventory Command</h1><p className="muted">Vehicle identity first. Upload source and time second. Internal IDs stay trace-only.</p></div><div style={{display:"flex",gap:8}}><Link className="addVehicleQuick" style={{margin:0,padding:"0 14px"}} href="/dealer/inventory/logs">Vehicle Logs</Link><Link className="addVehicleQuick" style={{margin:0,padding:"0 14px"}} href="/dealer/inventory/new">+ Add Vehicle</Link></div></div>
    <div className="leadInboxTabs">{[["all","All"],["published","Published"],["draft","Draft"],["archived","Archived"]].map(([k,l])=><button key={k} className={filter===k?"active":""} onClick={()=>chooseFilter(k)}>{l} · {k==="all"?items.length:items.filter(v=>String(v.status||"").toLowerCase()===k).length}</button>)}</div>
