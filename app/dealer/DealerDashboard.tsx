...
import{appointmentIntent,createdAtOf,leadScore,pipelineStages,sameLocalDay,sourceLabel,stageLabels,stageOf,when,type LeadRecord}from"./crmFilters";
...
export default function DealerDashboard(){
  ...
  const liveInventory=inventory.filter(v=>String(v?.status||"").toLowerCase()==="published");
  const sold=leads.filter(l=>stageOf(l)==="sold");
  const next=hotBuyers[0]||appointments[0]||newToday[0]||leads[0]||null;
  const nextMove=next?(appointmentIntent(next)?"Confirm appointment":stageOf(next)==="new"?"Make first contact":"Follow up now"):"No priority lead right now";
  ...
  const summaryTiles=[
    ...
    {label:"LIVE INVENTORY",value:liveInventory.length,sub:"CUSTOMER-VISIBLE CARS",href:"/dealer/inventory?status=published",tone:"green"},
    {label:"SOLD",value:sold.length,sub:"CLOSED OPPORTUNITIES",href:"/dealer/leads?stage=sold",tone:"white"}
  ];
  ...
  return <main className="salesCommandApp">
    ...
    <div className="salesCommandBody">
      ...
      <section className="salesPanel nextPanel">
        <header><div><span className="panelKicker">DO THIS NOW</span><h2>NEXT BEST MOVE</h2></div>{next&&<b className="priorityPill">{leadScore(next)} PRIORITY</b>}</header>
        {next?<div className="nextLead"><div className="hotFlag">HOT</div><div className="nextLeadCopy"><Link href={`/dealer/crm/${encodeURIComponent(next.id)}`}>{next.name||next.customerName||"Unnamed buyer"}</Link><p>{next.vehicleInterest||sourceLabel(next)}</p><div className="nextMeta"><span>{stageLabels[stageOf(next)]||stageOf(next)}</span><span>{when(createdAtOf(next))}</span></div><div className="recommendation"><small>WDCC RECOMMENDS</small><strong>{nextMove}</strong><p>Open the account and act while the buyer intent is still fresh.</p></div><div className="nextActions"><Link href={`/dealer/crm/${encodeURIComponent(next.id)}`}>OPEN ACCOUNT</Link>{next.phone&&<a href={`tel:${next.phone}`}>CALL</a>}{next.phone&&<a href={`sms:${next.phone}`}>TEXT</a>}</div></div></div>:<div className="salesEmpty">No lead needs immediate action.</div>}
      </section>
