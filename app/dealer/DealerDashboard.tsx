...
import {primaryVehiclePhoto} from "../../lib/vehiclePhotos";
...
const imageFor=(v:Vehicle)=>primaryVehiclePhoto(v);
  ...
  return <main className="targetDealerApp">
    ...
    <div className="targetDealerFrame">
      ...
      <section className="targetDealerMain">
        ...
        <div className="targetDealerLower"><section><header><b>Top Performing Vehicles</b><Link href="/dealer/inventory">View all inventory →</Link></header><div className="topVehicleTable"><div className="head"><span>VEHICLE</span><span>VIEWS</span><span>LEADS</span><span>STATUS</span></div>{topVehicles.map((v,i)=><Link href={v.id?`/vehicle/${encodeURIComponent(String(v.id))}`:"/dealer/inventory"} key={v.id||i}><span className="vehicleLabel">{imageFor(v)?<img src={imageFor(v)} alt=""/>:<span className="vehiclePhotoPending">PHOTO</span>}<b>{vehicleName(v)}</b><small>{money(v.price)}</small></span><span>{Number(v.views||0)}</span><span>{Number(v.leads||0)}</span><em>{String(v.status||"available").toUpperCase()}</em></Link>)}{!topVehicles.length&&<p className="emptyRow">No inventory yet.</p>}</div></section><section><header><b>Recent Activity</b><Link href="/dealer/leads">All activity →</Link></header><div className="recentActivity">{recent.map((l,i)=><Link href={`/dealer/crm/${encodeURIComponent(String(l.id))}`} key={l.id||i}><i>◎</i><span><b>{l.name||"New lead"}</b><small>{l.vehicleInterest||sourceLabel(l)||stageLabels[stageOf(l)]}</small></span><em>{when(createdAtOf(l))}</em></Link>)}{!recent.length&&<p className="emptyRow">No recent activity.</p>}</div></section></div>
