  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.main}>
      ...
      <section className={styles.gridTwo}>
        <div className={styles.panel}><div className={styles.panelHead}><div><span>DO THIS NOW</span><h2>Priority Buyers</h2></div><Link href="/dealer/leads">View all →</Link></div><div className={styles.priorityList}>{hot.length?hot.slice(0,5).map((lead:any,index:number)=><article className={styles.priorityCard} key={lead.id}><div className={styles.rank}>{index+1}</div><div className={styles.person}><div className={styles.personTop}><strong>{lead.name||"Unnamed lead"}</strong><span>{lead.priority>=70?"HOT":"ACTIVE"}</span></div><p>{lead.vehicleInterest||"General vehicle inquiry"}</p><small>{lead.kind||"lead"} · {ageLabel(lead.createdAt)}</small></div><div className={styles.action}><b>{lead.nextAction}</b><small>{lead.priority!=null?`Priority ${lead.priority}`:"Not scored"}</small><div>{lead.phone&&<a href={phoneHref(lead.phone)}>Call</a>}{lead.phone&&<a href={smsHref(lead.phone)}>Text</a>}<Link href={`/dealer/crm/${encodeURIComponent(lead.id)}`}>360</Link></div></div></article>):<div className={styles.empty}>No active leads yet. New website requests will appear here automatically.</div>}</div></div>
        <div className={styles.panel}><div className={styles.panelHead}><div><span>PIPELINE</span><h2>Lead → Close</h2></div><small>Live records only</small></div><div className={styles.pipeline}>{Object.entries(pipelineLabels).map(([stage,label])=><div key={stage}><div><span>{label as string}</span><b>{pipeline[stage]||0}</b></div><i><em style={{width:`${Math.min(100,((pipeline[stage]||0)/Math.max(1,summary.totalLeads||1))*100)}%`}}/></i></div>)}</div><div className={styles.momentum}><span>Momentum</span><strong>{summary.sold?`${summary.sold} sold`:`Learning from live activity`}</strong><small>WDCC will add response, show, close and gross scores as those outcomes are recorded.</small></div></div>
      </section>
      <section className={styles.panel}><div className={styles.panelHead}><div><span>LIVE CRM</span><h2>Customer Queue</h2></div><b>{leads.length} records</b></div><div className={styles.table}><div className={styles.tableHead}><span>Customer</span><span>Stage</span><span>Vehicle</span><span>Request</span><span>Last activity</span><span>Action</span></div>{leads.length?leads.map((lead:any)=><div className={styles.row} key={lead.id}><div><strong>{lead.name||"Unnamed"}</strong><small>{lead.phone||lead.email||"No contact"}</small></div><span className={styles.stage}>{pipelineLabels[lead.pipelineStage]||lead.pipelineStage}</span><span>{lead.vehicleInterest||"Open"}</span><span>{lead.kind||"lead"}</span><time>{ageLabel(lead.createdAt)}</time><Link href={`/dealer/crm/${encodeURIComponent(lead.id)}`}>Open 360 →</Link></div>):<div className={styles.empty}>No leads match this search.</div>}</div></section>
      <section className={styles.panel}><div className={styles.panelHead}><div><span>BUYER × INVENTORY</span><h2>Cars Ready To Sell</h2></div><Link href="/dealer/inventory">Manage inventory →</Link></div><div className={styles.cars}>{inventory.length?inventory.map((v:any)=>{const image=vehicleImageSource(v);return <article key={v.id}><div className={styles.carImage}>{image?<img src={image} alt="" onError={event=>{event.currentTarget.hidden=true}}/>:<span>PHOTO PENDING</span>}</div><div><small>{v.year} {v.make}</small><strong>{v.model} {v.trim||""}</strong><b>${Number(v.price||0).toLocaleString()}</b><span>{Number(v.mileage||0).toLocaleString()} mi</span></div><Link href={`/vehicle/${v.slug||v.id}`}>View vehicle →</Link></article>}):<div className={styles.empty}>No customer-visible vehicles.</div>}</div></section>
    </section>
  </main>;
}
