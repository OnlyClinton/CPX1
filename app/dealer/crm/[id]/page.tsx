...
export default function Customer360(){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.grid}><article className={styles.panel}><span className={styles.label}>CONTACT</span><dl><div><dt>Phone</dt><dd>{lead.phone||"Not provided"}</dd></div><div><dt>Email</dt><dd>{lead.email||"Not provided"}</dd></div><div><dt>Source</dt><dd>{lead.source||"Unknown"}</dd></div><div><dt>Created</dt><dd>{when(lead.createdAt)}</dd></div></dl></article><article className={styles.panel}><span className={styles.label}>BUYER REQUEST</span><dl><div><dt>Type</dt><dd>{lead.kind||"Lead"}</dd></div><div><dt>Vehicle</dt><dd>{lead.vehicleInterest||"Open to options"}</dd></div><div><dt>Preferred time</dt><dd>{lead.preferredTime||"Not specified"}</dd></div><div><dt>Status</dt><dd>{stageNames[lead.pipelineStage]||lead.status||"New"}</dd></div></dl></article>{lead.kind==="approval"&&<article className={styles.panel}><span className={styles.label}>FINANCING REQUEST</span><dl><div><dt>Monthly income</dt><dd>{lead.monthlyIncome||"Not provided"}</dd></div><div><dt>Down payment</dt><dd>{lead.downPaymentInterest||"Not provided"}</dd></div><div><dt>Referral source</dt><dd>{lead.referralSource||"Not provided"}</dd></div><div><dt>Privacy</dt><dd>Dealer access only</dd></div></dl></article>}<article className={`${styles.panel} ${styles.note}`}><span className={styles.label}>LATEST NOTE</span><p>{lead.message||"No note was submitted with this lead."}</p></article></section>
    <section className={styles.panel}><div className={styles.sectionHead}><div><span className={styles.label}>INVENTORY MATCH</span><h2>Vehicles to discuss</h2></div><Link href="/dealer/inventory">All inventory →</Link></div><div className={styles.cars}>{matches.length?matches.map((v:any)=>{const image=vehicleImageSource(v);return <article key={v.id}><div className={styles.photo}>{image?<img src={image} alt="" onError={event=>{event.currentTarget.hidden=true}}/>:<span>PHOTO PENDING</span>}</div><div><small>{v.year} {v.make}</small><strong>{v.model} {v.trim||""}</strong><b>${Number(v.price||0).toLocaleString()}</b><span>{Number(v.mileage||0).toLocaleString()} mi · ${Number(v.downPayment||0).toLocaleString()} down</span></div><Link href={`/vehicle/${v.slug||v.id}`}>View →</Link></article>}):<p>No direct vehicle match yet. Open the inventory to choose one.</p>}</div></section>
