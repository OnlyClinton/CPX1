...
export function Intro(){
  const[phase,setPhase]=useState<"enter"|"move"|"handoff"|"done">("enter");
  useEffect(()=>{
    if(sessionStorage.getItem("wdcc_intro_seen")||window.matchMedia("(prefers-reduced-motion: reduce)").matches){setPhase("done");return}
    sessionStorage.setItem("wdcc_intro_seen","1");
    const a=setTimeout(()=>setPhase("move"),520);
    const b=setTimeout(()=>setPhase("handoff"),1850);
    const c=setTimeout(()=>setPhase("done"),2850);
