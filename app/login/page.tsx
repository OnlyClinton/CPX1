...
export default function UnifiedLogin(){
  ...
  useEffect(()=>{
    ...
    fetch("/api/auth/session",{cache:"no-store",credentials:"include",signal:AbortSignal.timeout(6000)})
      ...
      .then(session=>{
        ...
        if(session?.session?.mustChangePassword===true||session?.user?.mustChangePassword===true){location.replace("/change-password");return;}
  ...
  async function submit(e:FormEvent){
    ...
    try{
      ...
      if(j?.mustChangePassword===true||j?.user?.mustChangePassword===true){
        setMessage("Choose your permanent password to continue.");
      ...
      if(!destination){
        ...
        if(s.ok&&sj?.authenticated){
          if(sj?.session?.mustChangePassword===true||sj?.user?.mustChangePassword===true){
            setMessage("Choose your permanent password to continue.");
            location.replace("/change-password");
            return;
          }
          destination=destinationForRole(sj?.user?.role);
        }
      }
