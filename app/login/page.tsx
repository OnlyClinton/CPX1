...
export default function UnifiedLogin(){
  ...
  return <main className="loginShell">
    ...
    <section className="formPanel"><form onSubmit={submit} aria-busy={busy}><span>AUTHORIZED ACCESS</span><h2>Sign In</h2><p className="introCopy">Use your WDCC administrator or dealer login.</p><label>LOGIN<input value={username} onChange={e=>setUsername(e.target.value)} autoComplete="username" autoCapitalize="none" spellCheck={false} inputMode="text" placeholder="admin or bigpussy" disabled={busy} required/></label><label>PASSWORD<div className="passwordRow"><input value={password} onChange={e=>setPassword(e.target.value)} type={showPassword?"text":"password"} autoComplete="current-password" disabled={busy} required/><button className="showPassword" type="button" onClick={()=>setShowPassword(v=>!v)} disabled={busy} aria-label={showPassword?"Hide password":"Show password"}>{showPassword?"HIDE":"SHOW"}</button></div></label><button className="submit" disabled={busy}>{busy?"VERIFYING…":"SIGN IN"}</button><div className="message" role="status" aria-live="polite">{message}</div><div className="portalLinks"><Link href="/">← Storefront</Link><span>Secure WDCC access</span></div></form></section>
    <style jsx>{`
