...
const session=String(process.env.SESSION_SECRET||'').length>=64;
...
console.log('VERIFY='+JSON.stringify({session,store,readable,revision,error}));
NODE
...
jq -e --arg sid "$STORE_ID" '.session==true and .store==$sid and .readable==true' /tmp/wdcc-envverify.json >/dev/null
