...
async function run(){
  ...
  for(const spec of [
    ...
    if(bootstrap){const br=await page.goto(bootstrap,{waitUntil:'networkidle'});if(!br||br.status()>=400)throw Error(`${spec.key}_BOOTSTRAP_${br?.status()}`);}
    ...
    const api={};if(sessionCookie){const paths=spec.key==='dealer'?['/api/crm/dashboard','/api/inventory','/api/leads']:['/api/admin/users'];for(const path of paths){const r=await ctx.request.get(base+path);api[path]=r.status();}}
