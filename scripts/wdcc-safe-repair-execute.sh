...
STOREFRONT_PROJECT="prj_We7xkAkB5Qy31Pt17USSkQFE0u7h"
DEALER_PROJECT="prj_fz5mN7Q5gImZ9UGpv1GDpHxPtLNB"
...
code="$(curl -sS -o /tmp/live-before.json -w '%{http_code}' "$LIVE/api/health?safe_direct=$GITHUB_RUN_ID")"
test "$code" = 200
...
console.log(JSON.stringify({ok:true,revision:Number(s.revision||0),users:s.users.length,leads:s.leads.length,vehicles:s.vehicles.length,qa:(s.vehicles||[]).filter(v=>stocks.has(String(v.stock||''))).map(v=>({id:v.id,stock:v.stock,status:v.status}))}));
NODE

source_project=''
for p in "$PHOENIX_PROJECT" "$DEALER_PROJECT" "$STOREFRONT_PROJECT"; do
  d="/tmp/wdcc-authority-$p"
  ...
  grep -q '^SESSION_SECRET=' "$d/env" || continue
  grep -q '^BLOB_READ_WRITE_TOKEN=' "$d/env" || continue
  if (set -a; . "$d/env"; set +a; node /tmp/probe-authority.mjs > "$d/probe.json" && jq -e '.ok==true' "$d/probe.json" >/dev/null); then
    cp "$d/env" /tmp/wdcc-authority.env
    cp "$d/probe.json" /tmp/wdcc-authority-probe.json
    source_project="$p"
    break
  fi
done

test -n "$source_project" || { echo 'P0:NO_WORKING_STATIC_AUTHORITY' >&2; exit 78; }
set -a; . /tmp/wdcc-authority.env; set +a
echo "::add-mask::$BLOB_READ_WRITE_TOKEN"
echo "::add-mask::$SESSION_SECRET"
printf 'AUTHORITY_PROJECT=%s\n' "$source_project" >/tmp/wdcc-safe-result.env
...
for i in $(seq 1 20); do
  code="$(curl -sS -o /tmp/candidate-before.json -w '%{http_code}' "$candidate/api/health?safe_direct=$GITHUB_RUN_ID&try=$i" || true)"
  if [ "$code" = 200 ] && jq -e --arg sha "$CANDIDATE_SHA" '.ok==true and .degraded==false and .storage=="token" and .state=="readable" and .commit==$sha' /tmp/candidate-before.json >/dev/null 2>&1; then healthy=1; break; fi
  sleep 2
done
cat /tmp/candidate-before.json
test "$healthy" = 1 || { echo 'P0:FROZEN_CANDIDATE_NOT_HEALTHY' >&2; exit 1; }
...
cat >/tmp/archive-exact-qa.mjs <<'NODE'
...
for(let attempt=1;attempt<=3;attempt++){
  ...
  try{
    await put(path,JSON.stringify(s,null,2)+'\n',{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'application/json',ifMatch:h.etag});
    result={ok:true,noOp:false,revision:s.revision,archived,backup}; break;
  }catch(e){if(e instanceof BlobPreconditionFailedError && attempt<3) continue; throw e;}
}
if(!result) throw Error('QA_ARCHIVE_NO_RESULT');
console.log(JSON.stringify(result));
NODE
GITHUB_RUN_ID="$GITHUB_RUN_ID" node /tmp/archive-exact-qa.mjs | tee /tmp/qa-cleanup.json
...
code="$(curl -sS -o /tmp/live-after.json -w '%{http_code}' "$LIVE/api/health?safe_direct_after=$GITHUB_RUN_ID")"
test "$code" = 200
jq -e --arg sha "$ROLLBACK_SHA" '.ok==true and .degraded==false and .commit==$sha' /tmp/live-after.json >/dev/null
code="$(curl -sS -o /tmp/candidate-after.json -w '%{http_code}' "$candidate/api/health?safe_direct_after=$GITHUB_RUN_ID")"
test "$code" = 200
jq -e --arg sha "$CANDIDATE_SHA" '.ok==true and .degraded==false and .state=="readable" and .commit==$sha' /tmp/candidate-after.json >/dev/null

echo 'SAFE_REPAIR_DIRECT=PASS'
