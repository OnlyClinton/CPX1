...
export PHOENIX_ID STORE_ID BASE RUN_ID
...
for(let attempt=1;attempt<=2;attempt++){
  const before=await readStable();const s=before.state;const lids=(s.leads||[]).filter(qLead).map(x=>x.id),vids=(s.vehicles||[]).filter(qVehicle).map(x=>x.id);const now=new Date().toISOString();
  if(!lids.length&&!vids.length){result={ok:true,noOp:true,revision:Number(s.revision||0),leadsRemoved:0,vehiclesArchived:0};break}
  const backup=`private/state/backups/platform-v3-pre-local-e2e-clean-r${Number(s.revision||0)}-${now.replace(/[:.]/g,'-')}-${crypto.randomUUID()}.json`;await put(backup,before.raw,{access:'private',addRandomSuffix:false,allowOverwrite:false,contentType:'application/json'});
...
# Run candidate locally as the Phoenix authority with the production OIDC/store binding and an ephemeral QA-only session secret.
env -u VERCEL_ORG_ID -u VERCEL_PROJECT_ID vercel env run --environment=production --token="$VERCEL_TOKEN" --scope="$TEAM" -- bash -c 'export SESSION_SECRET="$LOCAL_SESSION_SECRET"; export VERCEL_PROJECT_ID="$PHOENIX_ID"; export BLOB_STORE_ID="$STORE_ID"; export PORT=3200; npm start' > /tmp/wdcc-server.log 2>&1 &
SERVER_PID=$!
export SERVER_PID
...
for i in $(seq 1 60); do
  if curl -fsS "$BASE/api/health?local=$RUN_ID" -o /tmp/wdcc-health.json 2>/dev/null; then
    if jq -e '.ok==true and .degraded==false and .service=="wdcc-canonical-authority"' /tmp/wdcc-health.json >/dev/null 2>&1; then ready=1;break;fi
  fi
  sleep 1
done
test "$ready" = 1
...
(async()=>{
 const browser=await chromium.launch({headless:true});const ctx=await browser.newContext({viewport:{width:1440,height:1000},reducedMotion:'reduce'});const page=await ctx.newPage();const consoleErrors=[];page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text())});page.on('pageerror',e=>consoleErrors.push(String(e)));
 ...
 const inv0=await (await ctx.request.get(base+'/api/inventory?before='+Date.now())).json();assert(inv0.ok&&Array.isArray(inv0.items),'INVENTORY_CONTRACT');assert(inv0.items.every(v=>Number(v.year)>1900&&String(v.make||'').trim()&&String(v.model||'').trim()&&Number(v.price)>0&&!String(v.stock||'').toUpperCase().startsWith('R36TEST-')),'INVENTORY_PUBLIC_FILTER');if(inv0.items[0]){const vr=await ctx.request.get(base+'/vehicle/'+encodeURIComponent(inv0.items[0].id));assert(vr.status()===200,'VEHICLE_DETAIL_ROUTE')}
 async function lead(kind,path,source,phone){await page.goto(base+path+'?source='+encodeURIComponent(source),{waitUntil:'networkidle'});await page.getByLabel('Name').fill(`WDCC QA LOCAL ${kind} ${run}`);await page.getByLabel('Phone').fill(phone);await page.getByLabel('Email').fill(`qa-local-${kind}-${run}@invalid.example`);const vi=page.getByLabel('Vehicle of interest');if(await vi.count())await vi.fill('Local candidate QA vehicle');const consent=page.locator('input[name="consent"]');await consent.check();const pending=page.waitForResponse(r=>r.url().includes('/api/leads')&&r.request().method()==='POST');await page.getByRole('button',{name:'SEND REQUEST'}).click();const response=await pending;const j=await response.json();assert(response.ok()&&j.ok&&j.persisted,`LEAD_${kind}_FAILED`);assert(j.item.kind===kind&&j.item.source===source,`LEAD_${kind}_ATTRIBUTION`);return j.item}
 ...
 const dedupe=await ctx.request.post(base+'/api/leads',{headers:{'Content-Type':'application/json','Idempotency-Key':schedule.idempotencyKey},data:{qa:true,kind:'schedule',name:schedule.name,phone:schedule.phone,email:schedule.email,vehicleInterest:schedule.vehicleInterest||'Local candidate QA vehicle',message:'duplicate contract check',consent:true,source:'schedule-test-drive',idempotencyKey:schedule.idempotencyKey}});const dj=await dedupe.json();assert(dedupe.ok()&&dj.deduplicated===true&&dj.item.id===schedule.id,'LEAD_IDEMPOTENCY');
 const feed=await ctx.request.get(base+'/api/leads',{headers:{Cookie:`__Host-wdcc_session=${cookie}`}});const fj=await feed.json();assert(feed.ok()&&fj.ok,'DEALER_LEAD_FEED');for(const [id,source] of [[schedule.id,'schedule-test-drive'],[approval.id,'get-approved'],[contact.id,'call-sean']])assert((fj.items||[]).some(x=>x.id===id&&x.source===source),'LEAD_NOT_IN_FEED_'+source);
 const create=await ctx.request.post(base+'/api/inventory',{headers:{Cookie:`__Host-wdcc_session=${cookie}`,'Content-Type':'application/json'},data:{year:2019,make:'Toyota',model:`LOCAL-E2E-${run}`,trim:'QA',price:12345,downPayment:1500,mileage:54321,stock:`WDCC-QA-LOCAL-${run}`,description:'WDCC local candidate synthetic upload acceptance'}});const cj=await create.json();assert(create.status()===201&&cj.ok&&cj.item.id,'VEHICLE_CREATE');const vehicle=cj.item;const mediaPath=`media/wdcc/${vehicle.id}/qa-${run}.png`;const png=Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9WlV1GQAAAAASUVORK5CYII=','base64');await put(mediaPath,png,{access:'private',addRandomSuffix:false,allowOverwrite:true,contentType:'image/png'});
 const photo=await ctx.request.patch(base+'/api/inventory/'+encodeURIComponent(vehicle.id),{headers:{Cookie:`__Host-wdcc_session=${cookie}`,'Content-Type':'application/json'},data:{photoPathnames:[mediaPath],primaryPhotoPathname:mediaPath}});const phj=await photo.json();assert(photo.ok()&&phj.ok&&(phj.item.photoPathnames||[]).includes(mediaPath),'VEHICLE_PHOTO_CHECKPOINT');
 ...
 const publish=await ctx.request.patch(base+'/api/inventory/'+encodeURIComponent(vehicle.id),{headers:{Cookie:`__Host-wdcc_session=${cookie}`,'Content-Type':'application/json'},data:{status:'published'}});const pu=await publish.json();assert(publish.ok()&&pu.ok&&pu.item.status==='published','VEHICLE_PUBLISH');
 const dealerInv=await ctx.request.get(base+'/api/inventory',{headers:{Cookie:`__Host-wdcc_session=${cookie}`}});const dij=await dealerInv.json();assert((dij.items||[]).some(x=>x.id===vehicle.id&&x.status==='published'&&(x.photoPathnames||[]).length>0),'PUBLISHED_NOT_IN_DEALER_INVENTORY');
 const publicInv=await ctx.request.get(base+'/api/inventory?public='+Date.now());const pij=await publicInv.json();assert(!(pij.items||[]).some(x=>x.id===vehicle.id),'QA_VEHICLE_LEAKED_PUBLICLY');
 ...
 fs.writeFileSync('/tmp/wdcc-result.json',JSON.stringify({ok:true,routeResults,inventoryCount:inv0.items.length,leadIds:{schedule:schedule.id,approval:approval.id,contact:contact.id},vehicleId:vehicle.id,mediaPath,vehiclePublished:true,qaHiddenPublic:true,consoleErrors},null,2));await browser.close();
...
jq -e '.ok==true and .vehiclePublished==true and .qaHiddenPublic==true' /tmp/wdcc-result.json >/dev/null

# Explicit cleanup before normal exit; trap is a fallback for failures.
cleanup
trap - EXIT
