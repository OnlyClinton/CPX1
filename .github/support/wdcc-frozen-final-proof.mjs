...
try{
 ...
 const dash=await dealer.evaluate(()=>{const q=s=>document.querySelector(s),top=s=>q(s)?.getBoundingClientRect().top||0,img=q('.dcTop .brand>img'),style=getComputedStyle(img),r=img?.getBoundingClientRect(),before=getComputedStyle(q('.dcTop .brand'),'::before');return{metrics:getComputedStyle(q('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,ops:getComputedStyle(q('.opsCards')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,delta:Math.max(top('.inventoryOverview'),top('.recentVehicles'),top('.activityLocked'))-Math.min(top('.inventoryOverview'),top('.recentVehicles'),top('.activityLocked')),brandContent:style.content,brandDisplay:style.display,brandVisibility:style.visibility,brandOpacity:style.opacity,brandW:r?.width||0,brandH:r?.height||0,beforeDisplay:before.display,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(dash.metrics!==6||dash.ops!==4||dash.delta>3||!dash.brandContent.includes('wdcc-owner-logo')||dash.brandDisplay==='none'||dash.brandVisibility==='hidden'||Number(dash.brandOpacity)<.9||dash.brandW<45||dash.brandH<45||dash.beforeDisplay!=='none'||dash.overflow>2)fail('DEALER_DESKTOP',dash);
 ...
 const mdash=await md.evaluate(()=>{const q=s=>document.querySelector(s),img=q('.dcTop .brand>img'),style=getComputedStyle(img),r=img?.getBoundingClientRect(),before=getComputedStyle(q('.dcTop .brand'),'::before');return{side:getComputedStyle(q('.dcSide')).display,nav:getComputedStyle(q('.dashMobileNav')).display,metrics:getComputedStyle(q('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,brandContent:style.content,brandDisplay:style.display,brandVisibility:style.visibility,brandOpacity:style.opacity,brandW:r?.width||0,brandH:r?.height||0,beforeDisplay:before.display,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(mdash.side!=='none'||mdash.nav!=='grid'||mdash.metrics!==2||!mdash.brandContent.includes('wdcc-owner-logo')||mdash.brandDisplay==='none'||mdash.brandVisibility==='hidden'||Number(mdash.brandOpacity)<.9||mdash.brandW<38||mdash.brandH<38||mdash.beforeDisplay!=='none'||mdash.overflow>2)fail('DEALER_MOBILE',mdash);
