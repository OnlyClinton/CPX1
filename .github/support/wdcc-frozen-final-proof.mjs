...
try{
 ...
 const dash=await dealer.evaluate(()=>{const q=s=>document.querySelector(s),top=s=>q(s)?.getBoundingClientRect().top||0,brand=q('.dcTop .brand'),before=getComputedStyle(brand,'::before');return{metrics:getComputedStyle(q('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,ops:getComputedStyle(q('.opsCards')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,delta:Math.max(top('.inventoryOverview'),top('.recentVehicles'),top('.activityLocked'))-Math.min(top('.inventoryOverview'),top('.recentVehicles'),top('.activityLocked')),brandBg:before.backgroundImage,brandW:parseFloat(before.width)||0,brandH:parseFloat(before.height)||0,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(dash.metrics!==6||dash.ops!==4||dash.delta>3||!dash.brandBg.includes('wdcc-owner-logo')||dash.brandW<45||dash.brandH<45||dash.overflow>2)fail('DEALER_DESKTOP',dash);
 ...
 const mdash=await md.evaluate(()=>{const q=s=>document.querySelector(s),brand=q('.dcTop .brand'),before=getComputedStyle(brand,'::before');return{side:getComputedStyle(q('.dcSide')).display,nav:getComputedStyle(q('.dashMobileNav')).display,metrics:getComputedStyle(q('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,brandBg:before.backgroundImage,brandW:parseFloat(before.width)||0,brandH:parseFloat(before.height)||0,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(mdash.side!=='none'||mdash.nav!=='grid'||mdash.metrics!==2||!mdash.brandBg.includes('wdcc-owner-logo')||mdash.brandW<38||mdash.brandH<38||mdash.overflow>2)fail('DEALER_MOBILE',mdash);
