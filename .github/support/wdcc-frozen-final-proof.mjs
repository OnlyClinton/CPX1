...
try{
 ...
 const home=await d.evaluate(()=>{const q=s=>document.querySelector(s);return{headline:[...document.querySelectorAll('.rh-copy h1 span')].map(x=>(x.textContent||'').trim()),colors:[...document.querySelectorAll('.rh-copy h1 span')].map(x=>getComputedStyle(x).color),benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,featured:getComputedStyle(q('.rh-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,cards:q('.rh-grid')?.children.length||0,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(JSON.stringify(home.headline)!==JSON.stringify(['BAD CREDIT?','NO CREDIT?',"WE DON'T CARE."])||JSON.stringify(home.colors)!==JSON.stringify(['rgb(242, 31, 50)','rgb(22, 138, 244)','rgb(255, 255, 255)'])||home.benefits!==4||home.featured!==5||home.cards<5||home.finance!==4||home.trust!==4||home.overflow>2)fail('DESKTOP_HOME',home);
 ...
 const mobile=await m.evaluate(()=>{const q=s=>document.querySelector(s),header=q('[data-wdcc-public-chrome="header"]'),grid=q('.rh-grid'),first=grid?.children?.[0]?.getBoundingClientRect(),second=grid?.children?.[1]?.getBoundingClientRect(),third=grid?.children?.[2]?.getBoundingClientRect();return{headerH:header?.getBoundingClientRect().height||0,benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,visibleCards:[first,second,third].filter(r=>r&&r.left<innerWidth&&r.right>0).length,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(Math.abs(mobile.headerH-64)>2||mobile.benefits!==2||mobile.finance!==1||mobile.trust!==2||mobile.visibleCards<2||mobile.overflow>2)fail('MOBILE_HOME',mobile);
