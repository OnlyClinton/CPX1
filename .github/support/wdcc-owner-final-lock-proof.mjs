...
try{
 ...
 await goto(d,'/vehicle/proof-vdp?owner-review=0');await d.locator('.vehicleLayout').waitFor({state:'visible',timeout:10000});await waitPhotos(d,'.vehiclePhoto img',1);
 ...
 await goto(m,'/vehicle/proof-vdp?owner-review=0');await m.locator('.vehicleLayout').waitFor({state:'visible',timeout:10000});const mvdp=await m.evaluate(()=>({columns:getComputedStyle(document.querySelector('.vehicleLayout')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,gallery:document.querySelectorAll('.vehicleGallery button').length,facts:getComputedStyle(document.querySelector('.vehicleFacts')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,overflow:document.documentElement.scrollWidth-innerWidth}));if(mvdp.columns!==1||mvdp.gallery<2||mvdp.facts!==2||mvdp.overflow>2)fail('VDP_MOBILE',mvdp);result.public.vdpMobile=mvdp;await screenshot(m,'vehicle-detail-mobile');
