...
try{
 ...
 const d=await dc.newPage();watch(d);await ownerHome(d);
 const home=await d.evaluate(()=>{const q=s=>document.querySelector(s),w=q('.wdcc-header-wordmark'),wr=w.getBoundingClientRect(),hero=q('.rh-hero').getBoundingClientRect();return{text:(w.textContent||'').replace(/\s+/g,' ').trim(),wordmarkW:wr.width,wordmarkH:wr.height,cc:getComputedStyle(q('.wdcc-header-wordmark>strong>span')).color,heroH:hero.height,headline:[...document.querySelectorAll('.rh-copy h1 span')].map(x=>(x.textContent||'').trim()),headlineColors:[...document.querySelectorAll('.rh-copy h1 span')].map(x=>getComputedStyle(x).color),benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,featured:getComputedStyle(q('.rh-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,cards:q('.rh-grid').children.length,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(!home.text.includes('WDCC')||!home.text.includes("WE DON'T CARE CARS")||home.wordmarkW<118||home.wordmarkH<28||home.cc!=='rgb(239, 36, 53)'||Math.abs(home.heroH-500)>3||JSON.stringify(home.headline)!==JSON.stringify(['BAD CREDIT?','NO CREDIT?',"WE DON'T CARE."])||JSON.stringify(home.headlineColors)!==JSON.stringify(['rgb(242, 31, 50)','rgb(22, 138, 244)','rgb(255, 255, 255)'])||home.benefits!==4||home.featured!==5||home.cards!==5||home.finance!==4||home.trust!==4||home.overflow>2)fail('DESKTOP_STOREFRONT',home);
 result.desktop.home=home;await shot(d,'desktop-storefront');

 await go(d,'/vehicle/real-2004-nissan-350z?owner-review=1');await d.locator('.vehicleLayout').waitFor({state:'visible',timeout:15000});
 const vdp=await d.evaluate(()=>({cols:getComputedStyle(document.querySelector('.vehicleLayout')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,actions:[...document.querySelectorAll('.vehicleActionsPrimary .cta')].map(x=>(x.textContent||'').trim()),photo:!!document.querySelector('.vehiclePhoto img'),overflow:document.documentElement.scrollWidth-innerWidth}));
 if(vdp.cols!==2||JSON.stringify(vdp.actions)!==JSON.stringify(['SCHEDULE TEST DRIVE','CALL SEAN'])||!vdp.photo||vdp.overflow>2)fail('DESKTOP_VDP',vdp);result.desktop.vdp=vdp;await shot(d,'desktop-vdp');

 const dd=await dc.newPage();watch(dd);await wire(dd);await go(dd,'/dealer');await dd.locator('.targetDealerApp').waitFor({state:'visible',timeout:15000});
 const dealerDesktop=await dd.evaluate(()=>{const q=s=>document.querySelector(s),brand=q('.targetDealerTopBrand'),img=q('.targetDealerTopBrand img');return{before:getComputedStyle(brand,'::before').content,after:getComputedStyle(brand,'::after').content,imgDisplay:img?getComputedStyle(img).display:'missing',stats:getComputedStyle(q('.targetDealerStats')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,charts:getComputedStyle(q('.targetDealerCharts')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,lower:getComputedStyle(q('.targetDealerLower')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,quick:getComputedStyle(q('.targetDealerQuick')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,side:getComputedStyle(q('.targetDealerSide')).display,bottom:getComputedStyle(q('.targetDealerMobile')).display,overflow:document.documentElement.scrollWidth-innerWidth}});
 if(!dealerDesktop.before.includes('WDCC')||!dealerDesktop.after.includes("WE DON'T CARE CARS")||dealerDesktop.imgDisplay!=='none'||dealerDesktop.stats!==4||dealerDesktop.charts!==2||dealerDesktop.lower!==2||dealerDesktop.quick!==4||dealerDesktop.side==='none'||dealerDesktop.bottom!=='none'||dealerDesktop.overflow>2)fail('DEALER_DESKTOP_MOCK',dealerDesktop);
