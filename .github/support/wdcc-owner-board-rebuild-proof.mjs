...
try{
  ...
  const home=await d.evaluate(()=>{
    ...
    const grid=q('.rh-grid'),logo=q('img[data-wdcc-logo-art="owner-approved"]'),lr=rect(logo);
    return{
      logo:{src:logo?.getAttribute('src')||'',w:lr?.w||0,h:lr?.h||0,naturalWidth:logo?.naturalWidth||0,naturalHeight:logo?.naturalHeight||0},
      utilityH:rect(q('[data-wdcc-public-chrome="utility"]'))?.h||0,
      headerH:rect(q('[data-wdcc-public-chrome="header"]'))?.h||0,
      heroH:rect(q('.rh-hero'))?.h||0,
      heroLoaded:Boolean(q('.rh-hero-art')?.complete&&q('.rh-hero-art')?.naturalWidth>20),
      headline:[...document.querySelectorAll('.rh-copy h1 span')].map(e=>(e.textContent||'').trim()),
      colors:[...document.querySelectorAll('.rh-copy h1 span')].map(e=>getComputedStyle(e).color),
  ...
  if(home.logo.src!=='/wdcc-owner-logo'||home.logo.naturalWidth<20||home.logo.w<70||home.utilityH<28||home.headerH<70||!home.heroLoaded||home.heroH<500||JSON.stringify(home.headline)!==JSON.stringify(['BAD CREDIT?','NO CREDIT?',"WE DON'T CARE."])||JSON.stringify(home.colors)!==JSON.stringify(expectedColors)||home.benefits!==4||home.gridDisplay!=='grid'||home.gridTracks!==5||home.cards!==5||home.finance!==4||home.trust!==4||home.placeholders!==0||home.overflow>2)fail('DESKTOP_OWNER_BOARD',home);
  result.desktop.home=home;
  await d.screenshot({path:`${out}/desktop-home.png`,fullPage:true});
  ...
  if(di.tracks!==3||di.cards!==5||di.overflow>2)fail('DESKTOP_INVENTORY',di);
  ...
  const dash=await dd.evaluate(()=>({side:getComputedStyle(document.querySelector('.dcSide')).display,metricTracks:getComputedStyle(document.querySelector('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,opsTracks:getComputedStyle(document.querySelector('.opsCards')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,activityTop:document.querySelector('.activityLocked')?.getBoundingClientRect().top||0,inventoryTop:document.querySelector('.inventoryOverview')?.getBoundingClientRect().top||0,overflow:document.documentElement.scrollWidth-innerWidth}));
  if(dash.side==='none'||dash.metricTracks!==6||dash.opsTracks!==4||Math.abs(dash.activityTop-dash.inventoryTop)>3||dash.overflow>2)fail('DEALER_DASHBOARD',dash);
  result.dealer.desktopDashboard=dash;
  await dd.screenshot({path:`${out}/desktop-dealer-dashboard.png`,fullPage:true});

  await goto200(dd,'/dealer/inventory','DEALER_INVENTORY');
  await dd.locator('.inventoryRow').first().waitFor({state:'visible',timeout:10000});
  const dinv=await dd.evaluate(()=>({rows:document.querySelectorAll('.inventoryRow').length,head:getComputedStyle(document.querySelector('.inventoryHead')).display,overflow:document.documentElement.scrollWidth-innerWidth}));
  if(dinv.rows<5||dinv.head==='none'||dinv.overflow>2)fail('DEALER_INVENTORY',dinv);
  result.dealer.desktopInventory=dinv;
  await dd.screenshot({path:`${out}/desktop-dealer-inventory.png`,fullPage:true});

  await goto200(dd,'/dealer/inventory/new','DEALER_EDITOR');
  await dd.locator('.editVehicleApp').waitFor({state:'visible',timeout:10000});
  const edit=await dd.evaluate(()=>({steps:document.querySelectorAll('.stepper button').length,right:getComputedStyle(document.querySelector('.editRight')).display,topLogo:document.querySelector('.editTop .topBrand img')?.getBoundingClientRect().width||0,sideBrand:document.querySelector('.editSide>.editBrand')?getComputedStyle(document.querySelector('.editSide>.editBrand')).display:'none',overflow:document.documentElement.scrollWidth-innerWidth}));
  if(edit.steps!==5||edit.right==='none'||edit.topLogo<40||edit.sideBrand!=='none'||edit.overflow>2)fail('DEALER_EDITOR',edit);
  result.dealer.desktopEditor=edit;
  await dd.screenshot({path:`${out}/desktop-dealer-editor.png`,fullPage:true});
  await dctx.close();
  ...
  const mh=await m.evaluate(()=>{
    const q=s=>document.querySelector(s);
    const rect=e=>{const r=e?.getBoundingClientRect();return r?{x:r.x,y:r.y,w:r.width,h:r.height,right:r.right,bottom:r.bottom}:null};
    const grid=q('.rh-grid'),logo=q('img[data-wdcc-logo-art="owner-approved"]'),lr=rect(logo),cards=[...grid.children].map(rect),ctas=[...document.querySelectorAll('.rh-hero-actions .rh-btn')].map(rect),utility=q('[data-wdcc-public-chrome="utility"]');
    return{logo:{src:logo?.getAttribute('src')||'',w:lr?.w||0,h:lr?.h||0,cx:lr?lr.x+lr.w/2:0,naturalWidth:logo?.naturalWidth||0},utilityDisplay:utility?getComputedStyle(utility).display:'none',headerH:rect(q('[data-wdcc-public-chrome="header"]'))?.h||0,benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,gridDisplay:getComputedStyle(grid).display,cards,ctas,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,placeholders:grid.querySelectorAll('[role="img"][aria-label*="photo unavailable" i]').length,overflow:document.documentElement.scrollWidth-innerWidth};
  });
  if(mh.logo.src!=='/wdcc-owner-logo'||mh.logo.naturalWidth<20||mh.logo.w<58||mh.logo.w>66||Math.abs(mh.logo.cx-195)>4||mh.utilityDisplay!=='none'||mh.headerH<62||mh.headerH>66||mh.benefits!==4||mh.gridDisplay!=='flex'||mh.cards.length!==5||!mh.cards[2]||mh.cards[2].right>390||mh.ctas.length!==2||mh.ctas[1].y<=mh.ctas[0].bottom||mh.finance!==1||mh.trust!==2||mh.placeholders!==0||mh.overflow>2)fail('MOBILE_OWNER_BOARD',mh);
  result.mobile.home=mh;
  await m.screenshot({path:`${out}/mobile-home.png`,fullPage:true});

  await goto200(m,'/inventory?owner-review=1','MOBILE_INVENTORY');
  ...
  if(mi.tracks!==1||mi.cards!==5||mi.mediaH>205||mi.overflow>2)fail('MOBILE_INVENTORY',mi);
  ...
  const mdash=await md.evaluate(()=>({side:getComputedStyle(document.querySelector('.dcSide')).display,nav:getComputedStyle(document.querySelector('.dashMobileNav')).display,metrics:getComputedStyle(document.querySelector('.dashMetrics')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,overflow:document.documentElement.scrollWidth-innerWidth}));
  if(mdash.side!=='none'||mdash.nav!=='grid'||mdash.metrics!==2||mdash.overflow>2)fail('MOBILE_DASHBOARD',mdash);
  result.dealer.mobileDashboard=mdash;
  ...
  const medit=await md.evaluate(()=>({steps:document.querySelectorAll('.stepper button').length,fields:getComputedStyle(document.querySelector('.fieldGrid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,side:getComputedStyle(document.querySelector('.editSide')).display,right:getComputedStyle(document.querySelector('.editRight')).display,topBrand:Boolean(document.querySelector('.editTop .topBrand')),overflow:document.documentElement.scrollWidth-innerWidth}));
  if(medit.steps!==5||medit.fields!==2||medit.side!=='none'||medit.right!=='none'||!medit.topBrand||medit.overflow>2)fail('MOBILE_EDITOR',medit);
  result.dealer.mobileEditor=medit;
  await mctx.close();

  if(writes.length)fail('OWNER_BOARD_WRITE_REQUESTS',{writes});
  result.pass=true;
  fs.writeFileSync(`${out}/result.json`,JSON.stringify(result,null,2)+'\n');
  console.log('WDCC_OWNER_BOARD_PASS',JSON.stringify({desktopFeatured:home.gridTracks,mobileThirdRight:mh.cards[2].right,dealerMetrics:dash.metricTracks,mobileMetrics:mdash.metrics,writes:writes.length}));
}finally{
  await browser.close();
}
