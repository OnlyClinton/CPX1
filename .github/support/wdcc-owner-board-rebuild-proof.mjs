...
try{
  ...
  await skipIntro(d);
  await d.locator('.wdcc-wordmark').waitFor({state:'visible',timeout:10000});
  await d.locator('.rh-grid>article').first().waitFor({state:'visible',timeout:10000});
  await waitRecoveredPhotos(d,'.rh-grid>article img');
  const home=await d.evaluate(()=>{const q=s=>document.querySelector(s),grid=q('.rh-grid'),cards=[...grid.children].map(e=>{const r=e.getBoundingClientRect();return{w:r.width,h:r.height}}),headline=[...document.querySelectorAll('.rh-copy h1 span')].map(e=>(e.textContent||'').trim()),colors=[...document.querySelectorAll('.rh-copy h1 span')].map(e=>getComputedStyle(e).color);return{wordmark:(q('.wdcc-wordmark')?.textContent||'').replace(/\s+/g,' ').trim(),utilityH:R(q('[data-wdcc-public-chrome="utility"]'))?.h||0,headerH:R(q('[data-wdcc-public-chrome="header"]'))?.h||0,heroH:R(q('.rh-hero'))?.h||0,heroLoaded:Boolean(q('.rh-hero-art')?.complete&&q('.rh-hero-art')?.naturalWidth>20),headline,colors,benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,gridDisplay:getComputedStyle(grid).display,gridTracks:getComputedStyle(grid).gridTemplateColumns.split(/\s+/).filter(Boolean).length,cards,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,placeholders:grid.querySelectorAll('[role="img"][aria-label*="photo unavailable" i]').length,overflow:document.documentElement.scrollWidth-innerWidth}});
  if(!/WDCC.*WE DON'T CARE CARS/i.test(home.wordmark)||home.utilityH<28||home.headerH<70||!home.heroLoaded||home.heroH<520||JSON.stringify(home.headline)!==JSON.stringify(['BAD CREDIT?','NO CREDIT?',"WE DON'T CARE."])||home.benefits!==4||home.gridDisplay!=='grid'||home.gridTracks!==5||home.cards.length!==5||home.finance!==4||home.trust!==4||home.placeholders!==0||home.overflow>2)fail('DESKTOP_50869',home);
  ...
  if(dash.side==='none'||dash.metricTracks!==6||dash.opsTracks!==4||dash.overflow>2)fail('DEALER_10668_DESKTOP',dash);
  ...
  const edit=await dd.evaluate(()=>({right:getComputedStyle(document.querySelector('.editRight')).display,layout:getComputedStyle(document.querySelector('.editLayout')).gridTemplateColumns,overflow:document.documentElement.scrollWidth-innerWidth}));
  if(steps!==5||edit.right==='none'||edit.overflow>2)fail('DEALER_10667_DESKTOP',{steps,...edit});
  ...
  await skipIntro(m);
  await m.locator('.wdcc-wordmark').waitFor({state:'visible',timeout:10000});
  await m.locator('.rh-grid>article').first().waitFor({state:'visible',timeout:10000});
  await waitRecoveredPhotos(m,'.rh-grid>article img');
  const mobileHome=await m.evaluate(()=>{const q=s=>document.querySelector(s),grid=q('.rh-grid'),cards=[...grid.children].map(e=>{const r=e.getBoundingClientRect();return{x:r.x,right:r.right,w:r.width}}),utility=q('[data-wdcc-public-chrome="utility"]'),ctas=[...document.querySelectorAll('.rh-hero-actions .rh-btn')].map(e=>R(e));return{wordmark:(q('.wdcc-wordmark')?.textContent||'').replace(/\s+/g,' ').trim(),utilityDisplay:utility?getComputedStyle(utility).display:'none',headerH:R(q('[data-wdcc-public-chrome="header"]'))?.h||0,benefits:getComputedStyle(q('.rh-benefits')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,gridDisplay:getComputedStyle(grid).display,cards,ctas,finance:getComputedStyle(q('.rh-steps')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,trust:getComputedStyle(q('.rh-trust-grid')).gridTemplateColumns.split(/\s+/).filter(Boolean).length,placeholders:grid.querySelectorAll('[role="img"][aria-label*="photo unavailable" i]').length,overflow:document.documentElement.scrollWidth-innerWidth}});
  if(!/WDCC.*WE DON'T CARE CARS/i.test(mobileHome.wordmark)||mobileHome.utilityDisplay!=='none'||mobileHome.headerH<64||mobileHome.headerH>72||mobileHome.benefits!==4||mobileHome.gridDisplay!=='flex'||mobileHome.cards.length!==5||!mobileHome.cards[2]||mobileHome.cards[2].right>390||mobileHome.ctas.length!==2||mobileHome.ctas[1].y<=mobileHome.ctas[0].bottom||mobileHome.finance!==1||mobileHome.trust!==2||mobileHome.placeholders!==0||mobileHome.overflow>2)fail('MOBILE_50869',mobileHome);
